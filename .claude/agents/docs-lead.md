...
1. **Build the delta.** Read `CHANGELOG.md` (the newest version section and
   the few below it), then run `git log --oneline -- docs/ src/` to see what
   changed since the docs were last touched. Verify any CLI flag, config key,
   endpoint, or path against `src/hal0/` before it goes in a brief — never
   trust memory.
