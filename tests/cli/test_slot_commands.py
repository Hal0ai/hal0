...
a cold load (``providers.container._HEALTH_TIMEOUT_S``), and up to ~210s
for a restart (unload's bounded terminate + a full load). ``api_post``'s
default read timeout is 10s, an 18x mismatch: any lifecycle op over ~10s
used to raise ``ReadTimeout`` and exit 1 while the operation continued
and completed successfully server-side. These call sites must pass an
explicit timeout with headroom over the server's worst-case budget.
"""
