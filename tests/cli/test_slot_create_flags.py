...
def test_profile_flag_is_forwarded(captured_post: dict[str, Any]) -> None:
    """``slot create --profile`` overrides the API's type/device inference."""
    result = runner.invoke(
        slot_commands.app,
...
def test_profile_omitted_leaves_inference_to_the_api(captured_post: dict[str, Any]) -> None:
    """No ``--profile`` → no ``profile`` key; the API infers it from type/device.

    Sending an empty/placeholder profile here would defeat
    ``_normalize_create_body``'s inference, so the key must be absent.
    """
    result = runner.invoke(
        slot_commands.app,
...
def test_edit_profile_flag_is_forwarded(monkeypatch: pytest.MonkeyPatch) -> None:
    """``slot edit --profile`` PUTs the profile (repairs a profile-less slot)."""
    captured: dict[str, Any] = {}
