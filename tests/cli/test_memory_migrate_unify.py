...
``unify`` subcommand's ``--apply`` path gates on ``/version``'s
``features.document_export_api``/``document_import_api`` flags (the live
Hindsight instance here is still 0.7.2, which has neither) — so this test
pins that ``--apply`` refuses when those flags are absent/false, and
exercises the full export→import→poll→retag flow when they're present.
"""
