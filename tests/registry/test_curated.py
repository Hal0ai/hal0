...
def test_memory_pipeline_default_embed_model_is_pullable() -> None:
    ...
    The seeded embed lane (``hal0.config.seeds``) points at this id —
    without a curated ``hf_repo``/``hf_file`` for it, `hal0 model pull
    qwen3-embedding-0-6b-q8-0` 422s and the memory pipeline (Hindsight
    retain) has no embedding model to actually pull.
    """
    embed_id = "qwen3-embedding-0-6b-q8-0"
