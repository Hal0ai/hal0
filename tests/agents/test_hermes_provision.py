...
def test_install_phase_skips_venv_when_binary_exists(
    ...
    # The shipped hal0-memory plugin is dir-dropped; hal0-provider's source is
    # absent in-tree, so it's a skipped-step report line, not a copy.
    assert (hermes_home / "plugins" / "hal0-memory" / "__init__.py").is_file()
    assert any("hal0-provider" in s for s in out.details.get("plugins_skipped", []))
