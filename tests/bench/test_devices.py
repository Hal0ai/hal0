...
class TestHarnessConfig:
    def test_config_sh_hardcodes_no_device_node(self) -> None:
        """The literal regression guard for #1303.

        ``COMMON_RUN_FLAGS`` — the array that becomes the podman argv — must
        contain no device node and no GID at all; both now arrive through the
        resolver. (The allowlist below it legitimately names ``/dev/accel``
        as a permitted ROOT, which is why this asserts on the array block and
        not on the whole file.)
        """
        text = CONFIG_SH.read_text()
        code = "\n".join(line for line in text.splitlines() if not line.lstrip().startswith("#"))
        block = re.search(r"^COMMON_RUN_FLAGS=\((.*?)^\)", code, re.S | re.M)
        assert block, "COMMON_RUN_FLAGS array not found in config.sh"

        assert "/dev/" not in block.group(1)
        assert "--group-add" not in block.group(1)
        assert "amdgpu" not in code

    def test_config_sh_builds_run_flags_from_the_resolver(self, tmp_path) -> None:
        """End-to-end: source config.sh with the discovery roots pointed at a
        fake LXC layout and assert the podman argv it produces. Uses this
        interpreter as the harness python, so no install and no GPU needed."""
        kfd, dri = _lxc_dri(tmp_path)
        script = (
            f"set -uo pipefail\n"
        ...
        assert f"--device={kfd}" in flags
        assert f"--device={dri}/card1" in flags
        assert f"--device={dri}/renderD128" in flags
        assert not any("amdgpu" in f for f in flags)

    def test_config_sh_emits_no_device_flags_on_the_cpu_tier(self, tmp_path) -> None:
        """A CPU-tier sweep sources cleanly and asks podman for no devices."""
        script = (
            f"set -uo pipefail\n"
            f"MODEL_DIR={tmp_path}/models\n"
            f'source "{CONFIG_SH}"\n'
            f'echo "TIER=$BENCH_TIER"\n'
            f'printf "%s\\n" "${{COMMON_RUN_FLAGS[@]}}"\n'
        )
        proc = subprocess.run(
            ["bash", "-c", script],
            ...
            env={
                ...
                "HAL0_HOME": str(tmp_path / "hal0-home"),
                "HAL0_BENCH_PYTHON": sys.executable,
                "HAL0_BENCH_KFD_PATH": str(tmp_path / "absent-kfd"),
                "HAL0_BENCH_DRI_DIR": str(tmp_path / "absent-dri"),
            },
        )
    ...
    def test_config_sh_aborts_on_an_unusable_override(self, tmp_path) -> None:
        """Invalid device settings fail at source time — before any cell runs."""
        script = f'MODEL_DIR={tmp_path}/models\nsource "{CONFIG_SH}"\n'
        proc = subprocess.run(
            ["bash", "-c", script],
            capture_output=True,
            text=True,
            env={
                ...
                "HAL0_HOME": str(tmp_path / "hal0-home"),
                "HAL0_BENCH_PYTHON": sys.executable,
                "HAL0_BENCH_CARD_DEVICE": "/dev/dri/definitely-not-here",
            },
        )
