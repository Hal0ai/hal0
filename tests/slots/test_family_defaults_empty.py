...
_FAMILY_DEFAULTS = Path(__file__).resolve().parents[2] / "src/hal0/config/data/family_defaults.toml"
...
def test_family_defaults_parseable() -> None:
    ...
    raw = tomllib.loads(_FAMILY_DEFAULTS.read_text())
    assert isinstance(raw, dict), f"family_defaults.toml did not parse to a dict; got {type(raw)}"
