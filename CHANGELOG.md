...
- **`/etc/hal0/api.env` is no longer world-readable** (`0644` → `0640 root:hal0`). It carries `HAL0_ADMIN_KEY` / `HAL0_CLIENT_KEY`, so any local account could read the box's admin key. Worse, the ownership table is what `hal0 doctor perms --fix` enforces and the installer runs that on every install — so a box whose key had been *rotated* (rotation deliberately tightens to `0640`) was re-widened back to `0644` by the next doctor run. The group is pinned to the service group even on the root-era rollback table, because the hal0-run API reads this file in-process, not only as a systemd `EnvironmentFile`.
- **`hal0-systemctl` audited, no change needed** — the `rm -f` calls in the sudo-invoked wrapper were already quoted and already validate the slot id; the apparent finding was a grep hit inside its `usage:` heredoc.

### Fixed

- **MCP admin surface silently failed to mount on new installs.** A fresh install resolves `fastapi 0.140.0` / `starlette 1.3.1` (`pyproject.toml` pins only `fastapi>=0.115`). Starlette 1.x keeps `include_router`'d routes behind a wrapper entry instead of splicing them into `app.routes`, so the route-map autogen's flat walk produced an empty map, the catalog-drift guard raised, and `mount_mcp_servers` swallowed it as a warning — taking all ~82 admin tools with it. The walker now recurses (behavior unchanged on the older layout). Confirmed on a fresh Ubuntu 24.04 box: `hal0.mcp.mount_failed` before, `hal0.mcp.mounted admin=True` after.
- **Installer died on any container without `curl`.** Stock Ubuntu/Debian LXC templates ship without it, so the documented download-and-run-directly path failed at step 1/13 with "install it and re-run" (the `curl | bash` one-liner can't reach this — it needs curl to exist). Base prereqs are now auto-installed like `python3-venv`, podman and Node already were.
- **Container-runtime preflight stopped guessing.** It swallowed podman's own error and then blamed `nesting=1`/`keyctl=1`. On a fresh unprivileged CT that already had those flags set, podman failed with `socket: permission denied` — the LXC AppArmor profile confining its network setup — and the operator was sent to re-check correct config. The gate now always prints the runtime's error first, recognises the socket/namespace-denied signature, and names AppArmor and `/dev/net/tun`.
- **A fresh install no longer ends with `doctor perms` reporting drift.** The rendered-context atomic write used `os.replace`, which swaps in the temp file's inode and so discarded the destination's mode — Hermes provisioning re-rendered `STATE.md` right after the installer's `doctor perms --fix` backstop and reverted `0664` to the umask's `0644`. Mode is now preserved across the rename.
- **Slot writes reject model-owned capability keys at the shared seam**, not only at the HTTP boundary. `mtp`/`enable_thinking`/`vision` are model-owned; the guard ran in one route handler, so the stacks apply engine and both `SlotManager` write paths walked around it. The same defect had surfaced three times (slot drawer, stack-created slots, stack-reconciled slots). Applying a stack to an existing slot no longer writes those keys. A slot whose TOML still carries a folded-away value stays updatable — cleanup belongs to `hal0 slot migrate-caps`.
- **`installer/bench/generate_results_json.py` raised `UnboundLocalError`** — a shadowing `from datetime import datetime` inside the function made an earlier `datetime.now()` unbound. Went unnoticed because CI lints only `src`/`tests`.
