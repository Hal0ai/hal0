...
// through the same schema-driven AdvRow used by MemoryRerankerPanel; the
// five [honcho.llm] feature routes (deriver/dialectic/summary/dream/embedding)
...
// tab's HfTokenField: show configured/not-set from `set`, let the operator
...
function HonchoPanel({ registry }) {
  ...
  const [routesOpen, setRoutesOpen] = useStateSet(false);
            ...
            <div key={feature} style={{margin: "6px 12px 10px", paddingLeft: 10, borderLeft: "1px solid var(--line)"}}>
              ...
              {_honchoFeatureKeys(feature).map(k => (
                k.endsWith(".api_key_env")
                  ...
                  : <AdvRow key={k} dotKey={k} field={fields[k]} label={k.split(".").pop()} live={_getIn(live, k)} buf={buf[k]} onChange={onChange} registry={registry} />
              ))}
            </div>
          ))}
          <AdvRow
...
const ADV_OPTIONS = {
  ...
  "honcho.llm.deriver.transport": ["openai", "anthropic", "gemini"],
  "honcho.llm.dialectic.transport": ["openai", "anthropic", "gemini"],
  "honcho.llm.summary.transport": ["openai", "anthropic", "gemini"],
  "honcho.llm.dream.transport": ["openai", "anthropic", "gemini"],
  "honcho.llm.embedding.transport": ["openai", "anthropic", "gemini"],
};
...
function AdvRow({ dotKey, field, live, buf, onChange, registry, label: labelOverride }) {
