...
const { apiPostCalls, apiGetCalls, STATUS_RESPONSE } = vi.hoisted(() => ({
...
// POST, apiGet answers the reconciliation GET /pull/status the fix under test
// must call. Keep everything else (Hal0Error, etc.) real.
...
describe('useRunnerImagePullJob — stream-close reconciliation (#2120)', () => {
  ...
  it('lands failed state when the stream dies without a terminal event', async () => {
    ...
    await act(async () => {
      await snapshot.start('rocmfpx-combined')
    })
    ...
    expect(snapshot.error?.message).toMatch(/exit 125/)
    expect(snapshot.error?.code).toBe('runner_image.pull_failed')
