...
- **Render the /api/updates/check response as a rich panel + table.** (1 connections) — `src/hal0/cli/update_commands.py`
- **Die early on an editable/dev install — `hal0 update` can't swap the FHS tree.** (1 connections) — `src/hal0/cli/update_commands.py`
