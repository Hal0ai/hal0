...
- **`hal0 agent install hermes` (no flags) enables the gateway — parity     with ins** (1 connections) — `tests/cli/test_agent_install_hermes.py`
- **No provisioned hermes binary → nothing to wire the gateway onto.** (1 connections) — `tests/cli/test_agent_install_hermes.py`
- **Happy path: gateway install runs, the unit file lands, systemctl     enables + c** (1 connections) — `tests/cli/test_agent_install_hermes.py`
- **The secrets drop-in must be laid down BEFORE `hermes gateway install`     starts** (1 connections) — `tests/cli/test_agent_install_hermes.py`
