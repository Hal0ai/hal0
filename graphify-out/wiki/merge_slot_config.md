...
- **test_store_copy_safe_disable_still_writes()** (6 connections) — `tests/slot_config/test_merge_slot_config.py`
- **test_store_disable_does_not_rewrite_backend_device_model()** (6 connections) — `tests/slot_config/test_merge_slot_config.py`
...
- **Even when updates DO carry a model sub-table, the base's model dict     must not** (1 connections) — `tests/slot_config/test_merge_slot_config.py`
- **A pure disable carries NO ``[model]`` update, yet the slot's inherited     ``[mo** (1 connections) — `tests/slot_config/test_merge_slot_config.py`
- **SC-1 parity: a disable selection produces ``enabled=False`` and does     NOT rew** (1 connections) — `tests/slot_config/test_merge_slot_config.py`
...
- [SlotConfigStore](SlotConfigStore.md) (2 shared connections)
