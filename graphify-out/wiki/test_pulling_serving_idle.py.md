...
- **A pull_runner + cache-miss inserts PULLING before STARTING.** (1 connections) — `tests/slots/test_pulling_serving_idle.py`
- **A pull_runner with a cached model goes straight offline → starting.** (1 connections) — `tests/slots/test_pulling_serving_idle.py`
