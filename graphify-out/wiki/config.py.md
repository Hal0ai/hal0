...
- **Render a pydantic ValidationError into ``{field_path: message}``.** (1 connections) — `src/hal0/api/routes/config.py`
- **Pick the right hostname for the URLs we return.      The user reaches the API at** (1 connections) — `src/hal0/api/routes/config.py`
- **Return the API port the unit is bound to.      ``HAL0_PORT`` is the same env var** (1 connections) — `src/hal0/api/routes/config.py`
- **Return True if ``hal0-openwebui.service`` is active under systemd.      Uses ``s** (1 connections) — `src/hal0/api/routes/config.py`
