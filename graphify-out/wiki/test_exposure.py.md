...
- **Spot-check the read-only introspection + inference surface stays CLIENT.** (1 connections) — `tests/security/test_exposure.py`
- **With auth forced on, an ADMIN route 401s with no creds and 200s with the key.** (1 connections) — `tests/security/test_exposure.py`
