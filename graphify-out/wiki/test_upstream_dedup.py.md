...
- **Slots that haven't picked a model yet (empty ``model_default``)     are silently** (1 connections) — `tests/api/test_upstream_dedup.py`
- **Real on-disk slot TOMLs put the model id under ``[model] default``     (not the** (1 connections) — `tests/api/test_upstream_dedup.py`
