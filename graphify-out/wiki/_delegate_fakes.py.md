# _delegate_fakes.py

> 4 nodes · cohesion 0.50

## Key Concepts

- **_delegate_fakes.py** (13 connections) — `tests/harness/integration/_delegate_fakes.py`
- **upstream_base_environment_available()** (3 connections) — `tests/harness/integration/_delegate_fakes.py`
- **Fake execution-environment backends for δ-harness delegate_task coverage.  Backg** (1 connections) — `tests/harness/integration/_delegate_fakes.py`
- **Return True if the upstream Hermes checkout is on PYTHONPATH.      Used by the m** (1 connections) — `tests/harness/integration/_delegate_fakes.py`

## Relationships

- [FakeBackendResult](FakeBackendResult.md) (3 shared connections)
- [FakeDockerBackend](FakeDockerBackend.md) (2 shared connections)
- [FakeLocalBackend](FakeLocalBackend.md) (2 shared connections)
- [DelegateTaskSpec](DelegateTaskSpec.md) (2 shared connections)
- [_BackendContract](_BackendContract.md) (1 shared connections)
- [BackendInvocation](BackendInvocation.md) (1 shared connections)
- [FakeDelegateRunner](FakeDelegateRunner.md) (1 shared connections)

## Source Files

- `tests/harness/integration/_delegate_fakes.py`

## Audit Trail

- EXTRACTED: 18 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*