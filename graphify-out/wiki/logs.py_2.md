...
- **Suppress the narrow set of errors raised killing a dead subprocess.** (1 connections) — `src/hal0/slots/logs.py`
- **Return the last ``lines`` of ``unit``'s journal output (one-shot).      ``quiet`** (1 connections) — `src/hal0/slots/logs.py`
- **Follow ``unit``'s journal output, yielding filtered lines.      ``backfill_n`` r** (1 connections) — `src/hal0/slots/logs.py`
