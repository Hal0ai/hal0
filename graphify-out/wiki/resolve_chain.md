...
- **test_default_chains_shape()** (1 connections) — `tests/normalize/test_resolver.py`
...
- **hal0/brain is the dashboard agent-chat default. No installer seeds a     `brain`** (1 connections) — `tests/normalize/test_resolver.py`
- **hal0/utility chain is (utility, agent): when utility is unloaded but the     age** (1 connections) — `tests/normalize/test_resolver.py`
- **The npu virtual name matches ANY slot with device == "npu", regardless     of sl** (1 connections) — `tests/normalize/test_resolver.py`
