...
- **A kind='remote' upstream — container-backed when slot_name is set.** (1 connections) — `tests/dispatcher/test_container_gate.py`
- **A kind='slot' upstream (local slot path).** (1 connections) — `tests/dispatcher/test_container_gate.py`
...
- [_RecordingSlotManager](_RecordingSlotManager.md) (5 shared connections)
