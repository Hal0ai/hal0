...
## Corpus Check
- 1623 files · ~2,053,798 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29498 nodes · 56151 edges · 1345 communities (1204 shown, 141 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 6949 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `72821778`
...
## Communities (1345 total, 141 thin omitted)

### Community 0 - "useSlots.ts"
Cohesion: 0.03
Nodes (127): ADR-0022, useModels(), usePullJob(), DEFAULT_METRICS, fetchSlotsUnion(), FlagProvenance, IMAGE_PULL_TERMINAL, ImagePullSnapshot (+119 more)
...
### Community 3 - "Slot"
Cohesion: 0.03
Nodes (78): ModelCacheCheck, PullRunner, container_provider(), Return the process-wide ContainerProvider singleton., flm_id_to_tag(), Resolve a hal0 ``<tag>-FLM`` id back to FLM's native ``family:size`` tag.      I, Atomically write a slot TOML.      THE byte-level write path for ``/etc/hal0/slo, write_slot_toml() (+70 more)

### Community 4 - "test_updater.py"
Cohesion: 0.03
Nodes (149): _current_symlink(), _parse_manifest(), Return ``<usr_lib>/hal0-<version>/`` — where this release's tree lives., Return ``<usr_lib>/current`` — the atomic-swap target., Release manifest is missing required fields or has the wrong shape., Atomic self-update with cosign-verified releases and one-step rollback.      All, Validate ``raw`` against ReleaseManifest, raising UpdateManifestInvalid., UpdateManifestInvalid (+141 more)

### Community 5 - "BoardStore"
Cohesion: 0.03
Nodes (85): BoardStore, _coerce_board_list(), _coerce_card_list(), _new_card_id(), _now(), Any, Connection, Row (+77 more)

### Community 6 - "dashboard-redesign.jsx"
Cohesion: 0.03
Nodes (108): useActivityRecent(), CELL_DEFS, CELL_MAP, CellDef, CellId, DashLayout, DEFAULT_LAYOUT, LAYOUT_QUERY_KEY (+100 more)

### Community 7 - "SlotState"
Cohesion: 0.03
Nodes (105): SlotManager, Deep, id-keyed intent surface bound to one :class:`SlotManager`.      Constructe, The slot state-change stream — HARD REQ #5.          A thin wrap over the manage, SlotInterface, Push a record onto every active SSE subscriber queue., The model registry could not be consulted (outage, not a miss).      Raised by t, Async generator yielding every slot state transition as it happens.          Use, The deep intent surface (REWORK.md §E), lazily constructed.          ``inspect`` (+97 more)

### Community 8 - "Dispatcher"
Cohesion: 0.06
Nodes (74): LegacyResolutionFailed, Raised when the capability/path heuristics find no slot to serve a request., Dispatcher, DispatchError, NoRouteFound, Look up a registry binding for a model id.          Returns (upstream_name, upst, Base for any failure the dispatcher itself raises.      All subclasses use the `, No upstream could be found for the requested model id. (+66 more)
...
Cohesion: 0.03
Nodes (123): NamedTuple, FileSetEntry, FileSetPlan, One file of a planned file-set, ready for ``model_file`` insertion., The full set of files one model pull should download + register., Model, A model entry in the hal0 registry.      All fields are optional at construction, _comfyui_models_dir() (+115 more)

### Community 12 - "updater.py"
Cohesion: 0.04
Nodes (96): ConfigParseError, A config file is present but contains invalid TOML or fails validation., Tag-based release-policy derivation — the single source of truth for classifying, Raised when a tag does not match any supported release pattern., Immutable policy derived from a parsed release tag., ReleasePolicy, ReleaseTagError, _cache_dir() (+88 more)
...
Cohesion: 0.07
Nodes (61): ChatTemplate, useChatTemplates(), useMeta(), useMetaEnums(), useAddModelFromPath(), useModelDuplicate(), useModelInspect(), useModelSetDefault() (+53 more)

### Community 15 - "useModels.ts"
Cohesion: 0.04
Nodes (88): ConfigUrls, useConfigUrls(), AddFromPathRequest, fmtBytes(), fmtEta(), fmtSpeed(), HfSearchResponse, HfSearchResult (+80 more)

### Community 16 - "stacks.jsx"
Cohesion: 0.03
Nodes (93): URL-safe lowercase identifier used by the REST surface., api(), ComfyuiArbiter, ComfyuiEngineState, ComfyuiMemory, ComfyuiMode, ComfyuiStatus, ComfyuiSwitchover (+85 more)

### Community 17 - "HindsightProvider"
Cohesion: 0.06
Nodes (35): _agent_of(), bank_to_namespace(), HindsightProvider, _http_status(), namespace_to_bank(), _now(), _projects_of(), Any (+27 more)
...
Cohesion: 0.05
Nodes (82): useBackends(), CapabilitiesBag, CapabilityBackend, CapabilityCatalogItem, CapabilityRow, CapabilitySelection, useCapabilities(), useCapabilityApply() (+74 more)
...
Cohesion: 0.04
Nodes (59): ContainerProvider, Podman-container-per-slot inference backend.      One instance is shared across, Not applicable — systemd starts the container., Probe GET /health on the container port.          For llama-server slots /health, Poll /health until 200 or HEALTH_TIMEOUT_S exceeded.          Raises:, Push-driven failure detector, against a narrow host seam., SlotWatchdog, _make_provider_with_tmp_unit() (+51 more)

### Community 26 - "test_probe.py"
Cohesion: 0.04
Nodes (84): HardwareProbe, Detects hardware and produces a HardwareInfo snapshot.      The probe is intenti, _mk_run(), MonkeyPatch, Path, Unit tests for hal0.hardware.probe.  Covers:   - CPU parsing from /proc/cpuinfo, Build a fake _run() that dispatches by the first arg of cmd., No nvidia / no amd / no vulkan / no lspci → vendor=unknown empty info. (+76 more)

### Community 27 - "UpdateSeam"
Cohesion: 0.04
Nodes (78): _emit(), main(), _parse_result(), _pin_logs_to_stderr(), Any, CompletedProcess, UpdateSeam — the one narrow privileged seam self-update needs post-flip (#1464)., Build the one actionable privilege error, with the fix spelled out. (+70 more)
...
Cohesion: 0.07
Nodes (33): ConvergeReport, Any, Path, StackSlotEntry, StackApplyEngine — translate a StackConfig into an atomic slot-config change.  P, What converge() did, per slot. Failures are recorded, not raised., Reconcile a StackConfig onto slot TOMLs as one ChangeSet., Compute the post-state for ``stack``. Writes NOTHING.          Only entries that (+25 more)
...
Cohesion: 0.04
Nodes (116): agent_install(), agent_list(), agent_peers(), approvals_approve(), approvals_deny(), _install_hermes(), List installed bundled agents., Approve a pending agent action. (+108 more)
...
Cohesion: 0.05
Nodes (83): container_stub(), crud_app(), crud_client(), crud_models_root(), _events_since(), _max_event_id(), Any, FastAPI (+75 more)
...
### Community 42 - "v1.py"
Cohesion: 0.07
Nodes (71): DispatcherDep, _aggregate_models(), audio_speech(), audio_transcriptions(), chat_completions(), completions(), _dispatch_and_forward(), _dispatch_via_npu_trio() (+63 more)
...
Cohesion: 0.05
Nodes (71): check_hermes_ownership(), check_tree_group_share(), _dangling_registry_entries(), _deepest_mountpoint(), detect_editable_root(), doctor_models(), flm_mount_guard(), flm_store_divergence() (+63 more)

### Community 45 - "Upstream"
Cohesion: 0.08
Nodes (19): _model_info(), Any, Unit tests for ``hal0.providers.container.ContainerProvider``.  Issue #655 — tra, Regression guard for the 2026-06-15 chat@4096 incident: a slot whose     TOML pi, halo150/143 O8+O11: ONE render for every substrate — no version branch.      Nat, spec-hw-slot-ownership §2: the slot owns NGL + THREADS; the model no     longer, Test shim: render a plan to Quadlet ``.container`` text (was ``_render_unit_from, Loopback publish is derived from port + empty network_mode by the         render (+11 more)
...
Cohesion: 0.06
Nodes (73): AbstractEventLoop, _auto_resume_interrupted_pulls(), _boot_audit_store(), _boot_background_tasks(), _boot_capabilities(), _boot_dispatcher(), _boot_mcp_memory_call(), _boot_memory_dispatch() (+65 more)
...
Cohesion: 0.06
Nodes (27): _model(), ModelRegistry, Path, Unit tests for hal0.registry.sqlite_store.SqliteModelRegistry (ML-1).  Mirrors t, ON DELETE CASCADE must actually fire (foreign_keys=ON on every         connectio, created_at must not advance on update — only updated_at does., upstream_url must ride in `extra`, not get accidentally routed         into the, All-None ModelDefaults() must read back as None, not as an         object with e (+19 more)
...
Cohesion: 0.03
Nodes (74): RuntimeFamily, SlotType, create_profile(), delete_profile(), export_profile(), get_profile(), import_profile_route(), list_profiles() (+66 more)

### Community 58 - "CapabilityOrchestrator"
Cohesion: 0.05
Nodes (37): Fire-and-forget trigger to refresh the Hermes live-context files.  Called from t, Spawn a detached ``hal0-agent <agent_id> render-context``. Never raises., spawn_context_refresh(), Return the TTS slot profile a device selection implies.      The engine switch:, tts_profile_for_device(), child_to_slot(), legal_children(), Any (+29 more)
...
Cohesion: 0.03
Nodes (54): MemoryEmbeddingConfig, Any, [memory.embedding] section of hal0.toml — Hindsight-era rerank knobs.      ADR-0, Pydantic model for a single slot's TOML config (slots/<name>.toml).      Fields, Pull a `[server]` TOML table out of the loader's `extra` catch-all.          ``h, Pull a `[npu]` TOML table out of the loader's `extra` catch-all.          Mirror, Pull an `[image]` TOML table out of the loader's `extra` catch-all.          Mir, Read-only promotion shim: derive ``device`` from a legacy         on-disk ``back (+46 more)
...
Cohesion: 0.04
Nodes (73): One normalised row of an HF repo tree listing., RawTreeEntry, Any, Connection, Model, Path, Row, SqliteModelRegistry — SQLite-backed model catalog (ML-1 pilot).  Drop-in replace (+65 more)
...
Cohesion: 0.06
Nodes (56): Model ids referenced by any configured slot or any stack.      Union of: each sl, referenced_model_ids(), add_from_path(), auto_scan_and_register(), cascade_delete_model(), clear_slot_default(), comfyui_category(), commit_scan_rows() (+48 more)
...
Cohesion: 0.03
Nodes (87): _QueueItem, Path, _null_model_hw_columns(), NULL the ``n_gpu_layers``/``preferred_runner`` columns for ``model_ids``.      B, connect(), db_path(), Connection, Path (+79 more)

### Community 75 - "loader.py"
Cohesion: 0.04
Nodes (67): OSError, config_validate(), Validate all config files against the current schema., ConfigError, ConfigNotFound, _find_manifest_path(), _flatten_slot_toml(), list_agent_configs() (+59 more)

### Community 76 - "_RecordingSlotManager"
Cohesion: 0.04
Nodes (81): _filter_response_headers(), AsyncClient, Response, StreamingResponse, Wrap _forward_plain in SlotManager.serving for the slot's lifetime.          Non, Drop hop-by-hop and length headers so Starlette can recompute them., The chosen upstream couldn't be reached (timeout, connection refused, etc.)., The target slot is mid-swap — model is starting/loading/unloading.      Raised b (+73 more)

### Community 77 - "test_argv.py"
Cohesion: 0.12
Nodes (25): normalize_argv(), Drop every denylisted flag (and its value) from ``tokens``.      The healing cou, Dedup ``tokens`` keeping the last occurrence of each scalar/bool flag.      Effe, strip_managed_flags(), Unit + golden-parity tests for hal0.slots.argv.normalize_argv.  The golden fixtu, Last value following ``flag`` in ``tokens`` (the effective value)., spec-hw-slot-ownership §5 partition set: the grid-owned hardware flags     (devi, Token-exact matching: --model_path / --threads-batch are NOT --model /     --thr (+17 more)

### Community 78 - "load_profiles_config"
Cohesion: 0.07
Nodes (28): load_profiles_config(), Load and validate /etc/hal0/profiles.toml.      Returns a :class:`ProfilesConfig, Look up a named profile in the profiles.toml catalog.      Shared by every provi, resolve_profile(), _base_profile_for_backend answers the backend→base question using the     1.0 ca, test_base_profile_for_backend_is_non_mtp(), Path, Unit tests for the profile catalog — schema, loader, and flag resolver.  Targete (+20 more)
...
Cohesion: 0.03
Nodes (62): _mock_response(), model_info(), provider(), Any, Unit tests for FLMProvider.  Two-tier readiness (Option A — NPU double-free avoi, Bundled default when no slot override / env / manifest pin applies.      §7.1b /, §7.1b / ML-4 fix: FLM previously ignored slot.image entirely., The image bundles FLM; do not host-mount a binary tree on top of it.      Earlie (+54 more)
...
Cohesion: 0.03
Nodes (36): react, AGENT_TAB_DEFS, _agentRoute(), _agentTabs(), AgentView(), ADR-0023, Markdown(), mdInline() (+28 more)
...
Cohesion: 0.02
Nodes (80): BaseModel, PersonaApprovalUpdate, Partial ``[persona.approval]`` patch. Omitted fields are unchanged., _SelectionItem, DiagnosisOut, DoctorResponse, EvidenceOut, get_doctor() (+72 more)
...
Cohesion: 0.06
Nodes (52): _active_persona_render(), ApiServerKeyResult, BootstrapState, _brain_profile_config_path(), _default_mcp_servers(), ensure_gateway_api_server_key(), _fetch_model_contexts(), _generate_api_server_key() (+44 more)
...
### Community 120 - "Any"
Cohesion: 0.07
Nodes (49): _build_brain_identity_card(), _build_identity_card(), _collect_capability_rollup(), _collect_chat_slots(), _fetch_slots(), _find_named_ready_slot(), _find_slot(), _hal0_version_string() (+41 more)
...
### Community 124 - "test"
Cohesion: 0.04
Nodes (11): test, MOBILE, CURATED, UPSTREAM_ROW, Indicator, LEGACY_LIVE_STATES, NPU, BASE_SLOTS (+3 more)
...
Cohesion: 0.05
Nodes (42): lane_slot_pin(), preferred_slot(), Any, rank_slot_name(), Which slot a request actually asked for, and how to choose between slots (#1418), Record that this request resolved to the ``slot_name`` lane., The lane-pinned slot name for this request, or ``""``.      Never raises and nev, Selection key for one slot — lower sorts first.      ``(lane-miss, health-rank)` (+34 more)
...
Cohesion: 0.07
Nodes (40): AgentList, AgentRecord, AgentRestartResult, AgentSkill, AgentSkillsResponse, ApprovalEntry, ApprovalList, BUNDLED_MCP_IDS (+32 more)
...
### Community 157 - "Path"
Cohesion: 0.06
Nodes (48): _apply_config_set(), _copy_plugin_tree(), _copy_wrapper(), _dirs_identical(), _ensure_hermes_config(), _fmt_config_value(), _hal0_service_home(), _hal0_subprocess_env() (+40 more)
...
Cohesion: 0.05
Nodes (57): _flatten_for_ui(), Project HardwareInfo into the fields the Vue Hardware view expects.      The das, load_hardware_info(), Load /etc/hal0/hardware.json and return a validated HardwareInfo.      Returns t, GPUInfo, HardwareInfo, NPUInfo, One detected NPU (AMD XDNA / future vendors). (+49 more)
...
Cohesion: 0.06
Nodes (38): agent_log(), _agent_provision_state_file(), agent_reprovision(), agent_status(), agent_upgrade(), _approval_summary(), approvals_list(), bootstrap_hermes() (+30 more)

### Community 165 - "api_get"
Cohesion: 0.11
Nodes (27): auth_require(), auth_rotate(), KeyTier, hal0 auth subcommands — thin HTTP client to /api/auth/*.  §5.2 of the R5 sync as, Persist the [security].require_auth posture (PUT /api/auth/require)., Rotate the admin or client box key (POST /api/auth/rotate).      Mints a fresh k, RequireToggle, model_assign() (+19 more)
...
Cohesion: 0.05
Nodes (49): app_install(), app_list(), app_uninstall(), ``hal0 app`` subcommands — deferred install/uninstall verbs for optional apps., List known apps and their systemd enabled/active state., Stop + disable an app installed via `hal0 app install`.      Only tears down the, Install + enable an app that was skipped at install time.      Runs the identica, Best-effort ``systemctl is-<prop> <unit>`` — returns the raw stdout     (e.g. 'a (+41 more)

### Community 173 - "test_profile_derive.py"
Cohesion: 0.07
Nodes (47): derive_device(), derive_profile(), npu_takes_utility(), Auto-derive a slot's device + profile from the hardware probe (design D4).  Maps, Return a ``SEED_PROFILES`` name for a (capability, device) pair.      Reads the, True when the NPU claims the ``utility`` role on this box.      When True, the f, Return a ``DeviceLiteral`` for the capability, or None to skip it.      None mea, Parity/regression lock for the device→profile derivations (finding PS-4).  The p (+39 more)
...
Cohesion: 0.04
Nodes (51): useComfyui(), bundleNameOr(), InstallState, InstallStateBundle, useInstallState(), DEMO_MIGRATION_REPORT, EMPTY_MIGRATION_REPORT, MigrationFlagConflict (+43 more)
...
Cohesion: 0.06
Nodes (46): _build_brain_profile_mcp_servers(), _deep_merge(), _live_resolve_enabled(), _load_agent_allowlist(), _merge_config_yaml_layers(), path_is_writable(), _phase_brain_profile_mcp_wire(), _phase_brain_profile_seed() (+38 more)
...
Cohesion: 0.09
Nodes (38): app(), app_with_slot(), _Body, client(), _Color, FastAPI, Hal0Error, MonkeyPatch (+30 more)
...
Cohesion: 0.14
Nodes (18): Hold the coarse cross-process lock for ALL slots/*.toml writes.      Historicall, slot_write_lock(), apply_preferred_profile(), defuse_stale_mtp_on_swap(), preferred_profile_for(), profile_fits_slot(), ProfileAdoptHost, Any (+10 more)
...
### Community 221 - "die"
Cohesion: 0.10
Nodes (35): bank_consolidate_cmd(), bank_delete_cmd(), bank_export_cmd(), bank_import_cmd(), bank_list_cmd(), bank_stats_cmd(), _emit(), profile_get_cmd() (+27 more)

### Community 222 - "run_migrations"
Cohesion: 0.08
Nodes (27): _deep_copy_dict(), latest_version(), MigrationError, Any, hal0.config.migrations — versioned config migration transforms.  # TIER3: config, # NOTE: downgrade migrations are explicitly unsupported in v1., Read ``meta.schema_version`` from a raw config dict, defaulting to 1., Cheap deepcopy for the TOML-shaped dicts we deal with.      TOML decodes to str (+19 more)
...
Cohesion: 0.09
Nodes (28): Hal0Reranker, Async reranker over hal0-api's OpenAI surface (Cohere-style ``/v1/rerankings``)., Fake404HindsightClient, FakeHindsightClient, FakeReranker, HindsightProvider unit tests — bank mapping + fan-out (P1)., Reverses input order so we can prove the merge re-ranked the union., Records calls; returns canned recall/retain/delete results. (+20 more)
...
### Community 239 - "_slot"
Cohesion: 0.14
Nodes (14): _agg(), FakeSlotManager, FakeUpstreams, no_mem(), MonkeyPatch, SimpleNamespace, Slot, Unit tests for hal0.slot_view — SlotViewAggregator + per-concern functions.  Iss (+6 more)
...
Cohesion: 0.07
Nodes (40): Path, TestClient, Regression: the slot-edit Save's ``PATCH /defaults`` must not wipe the     model, #585: the dashboard writes ``ctx_size``; it must persist as the canonical     ``, When the NPU LLM anchor has a model bound, every trio slot gets coresident_group, idle_timeout_s / workers / llamacpp_args ride along on /api/slots.      The on-d, GET /api/slots/{name} is enriched same shape as the list endpoint., Labels list is omitted (not empty) when the slot config has no     ``model.label (+32 more)
...
Cohesion: 0.10
Nodes (13): Any, Lock, Slot, Push-driven failure detector (P3-slots §1b-watchdog).  ``SlotWatchdog`` polls a, Spawn or cancel the per-slot fail-watcher to match ``new_state``.          Live, Flip a confirmed-faulted slot to ``to_state`` under the per-slot lock., True when the model server still fails its /health probe., True when the container unit still reads inactive to systemd. (+5 more)

### Community 248 - "write_toml_atomic"
Cohesion: 0.11
Nodes (20): Path, Read and mutate the stack catalog through one interface., StacksCatalog, Path, Unit tests for the active-stack pointer + drift detection.  Targeted file run:, PS-5 part 2 — a converge whose per-slot loads failed must not read clean.      T, _saber(), _slots_dir() (+12 more)
...
Cohesion: 0.05
Nodes (65): MigrationPlan, _apply_store_change(), _config_to_dict(), ConfigInvalidError, _deep_merge(), get_apply_plan(), get_model_store(), get_settings() (+57 more)
...
Cohesion: 0.10
Nodes (32): NextStep, _diagnose_audit_rows(), _diagnose_migration(), _diagnose_models(), _diagnose_profiles(), Diagnosis, Assemble the §21.4 ``HAL0-MODEL-*`` Diagnosis rows from the evidence     ``docto, One ``HAL0-MIGRATION-PENDING`` row (always ``warn`` — operator-initiated),     o (+24 more)
...
Cohesion: 0.05
Nodes (54): _classify_flm_model(), _ensure_flm_models_dir(), ensure_host_flm_store_link(), _extract_json_object(), flm_host_async_spawn(), flm_host_spawn_kwargs(), flm_pull_command(), flm_served_models() (+46 more)
...
Cohesion: 0.07
Nodes (33): build_per_slot(), CapacityProbeError, CapacitySnapshot, _ctx_tokens_for(), estimate_file_size_kv_mb(), _kv_estimate_mb(), Any, Slot capacity snapshot.  CapacitySnapshot is the single-source view of available (+25 more)
...
Cohesion: 0.05
Nodes (54): _active_hal0_units(), _backup_slot_state(), _migrate_id_keying_dry_run_plan(), Any, CompletedProcess, Path, One-shot: mtp/enable_thinking/vision stick to MODELS (spec-hw-slot-ownership §1), Every hal0-owned systemd unit currently active: ``hal0-api.service``     plus an (+46 more)
...
Cohesion: 0.16
Nodes (29): FLMProvider, Provider for the AMD NPU FLM backend.      Two-tier readiness (Option A — NPU do, _model_info(), Any, FLM container_spec: [npu] toggles + model-cache default (Phase A)., Isolated via ``tmp_hal0_home`` (tests/conftest.py): without it this     reads th, [models].flm_store must reach the container mount without the env var.      Regr, Spec build must mkdir the bind source so podman never sees ENOENT. (+21 more)
...
Cohesion: 0.09
Nodes (13): emitSseTyped(), emitWs(), installWsHarness(), waitForWs(), NOTE: addInitScript serialises the function — it must not close over, body(), engine(), pane() (+5 more)
...
Cohesion: 0.15
Nodes (24): device_to_backend(), Map hal0's ``device`` enum onto the recipe+backend pair.      Args:         devi, _base_profile_for_backend(), _cfg_effective_backend(), check_default_uniqueness(), check_npu_exclusivity(), _iter_peer_configs(), Any (+16 more)
...
Cohesion: 0.06
Nodes (43): npu_trio_slot_root(), coresident_group must key off device==npu, not the legacy slot names.      Deplo, Lay down the NPU FLM trio (agent + stt-npu + embed-npu) on disk., A model-less NPU LLM anchor → no trio markers on the siblings (#1369).      This, #1369: membership follows the anchor, with no per-sibling opt-out.      The shad, spec-hw-slot-ownership §1: a stray slot-level ``enable_thinking`` (pre-     migr, Slot with no [server] table → payload's llamacpp_args is null.      Mirror the e, rope_freq_base set on disk → exposed in /api/slots payload. (+35 more)
...
Cohesion: 0.09
Nodes (24): _atomic_write(), _atomic_write_if_changed(), content_hash(), _detect_foreign_gateways(), _disable_honcho_hermes_host(), _gateway_dropin_body(), GatewayDropinResult, _privileged_systemctl() (+16 more)
...
### Community 455 - "Any"
Cohesion: 0.15
Nodes (10): MapContainerProvider, _npu_anchor_cfg(), Any, Per-slot ``is_active`` — the trio anchor runs, shadows have no unit., #733: embed/stt shadow slots have no unit/container of their own —     the npu a, container_enrichment's stopped-vs-crashed escalation shells out         to a rea, #1369: shadows carry placeholder models, so there is no per-shadow         opt-o, A model-less anchor isn't running FLM, so it can't serve shadows. (+2 more)
...
Cohesion: 0.13
Nodes (18): _canon(), _dedup(), _deny_slot_hardware_flags(), _is_flag(), NormalizedArgv, _Pair, normalize_argv — the single dedup/last-wins pass for llama-server argv.  The lau, Result of :func:`normalize_argv`.      ``argv`` is the deduped token list. ``rem (+10 more)
...
### Community 566 - "Enum"
Cohesion: 0.19
Nodes (15): Enum, config_edit(), config_migrate(), _config_path(), config_show(), ConfigFile, _hal0_toml_path(), Path (+7 more)
...
Cohesion: 0.12
Nodes (16): FlagProvenance, One surviving flag and the input segment it was resolved from., Resolve ordered ``(source_label, tokens)`` segments into one deduped argv., resolve_argv(), A slot's real extra_args (bench tuning, no managed flags) passes through., A managed flag in a non-``extra_args`` (trusted) segment is not screened.      `, A model's free-form ``defaults.extra_args`` (the ``model_extra_args``     segmen, The ``-ngl`` hal0 computes from the schema field ``defaults.n_gpu_layers``     r (+8 more)
...
Cohesion: 0.36
Nodes (4): Serialise a real Slot snapshot into the API shape.      Adds ``kind="local"`` so, serialize_slot(), _slot(), TestSerializeSlot
...
Cohesion: 0.09
Nodes (23): Confidence, Diagnosis, Evidence, hardwareEvidence(), mb(), NextStep, overallVerdict(), useDiagnoses() (+15 more)
...
Cohesion: 0.25
Nodes (8): _atomic_symlink_swap(), Atomically point ``link_path`` at ``new_target``.      Returns the path the syml, First swap creates the symlink; prior is None., A second swap returns the prior target and points at the new one., After 50 rapid swaps no .swap-* turds remain in the install root.      Stress-te, test_atomic_symlink_swap_chaos_no_temp_left(), test_atomic_symlink_swap_creates_link(), test_atomic_symlink_swap_replaces_existing()
...
Cohesion: 0.33
Nodes (5): _capability_slot_for_path(), Resolve a path-pinned capability slot name (embed/rerank/tts/img).      Mirrors, DR-1: wake an idle-EVICTED path-pinned capability slot before dispatch.      The, _wake_capability_slot(), Lazily-constructed exclusive-GPU arbiter (llm ⇄ img groups).          State pers
...
### Community 1280 - ".argvs"
Cohesion: 0.33
Nodes (3): The canonical `hermes` wrapper must NOT export/pin HERMES_HOME at     all — the, Every recorded argv — the assertion surface for the seam tests., test_hermes_wrapper_does_not_pin_hermes_home()
...
- **Why does `create_app()` connect `create_app` to `settings.py`, `_refresh_model_cache_on_ready`, `EventBus`, `ApprovalQueue`, `test_hf_routes.py`, `test_stacks_routes.py`, `test_auth_core.py`, `conftest.py`, `GpuImageMode`, `isolated_client_with_root`, `test_dashboard_layout.py`, `test_route_collisions.py`, `test_memory_admin_routes.py`, `test_slots_policy.py`, `test_memory_delete_auth.py`, `test_settings_routes.py`, `test_models_crud.py`, `types.py`, `test_admin_route_map_nested.py`, `test_updater_routes.py`, `__init__.py`, `model_store_root`, `mcp_mount.py`, `test_config_model_roots.py`, `test_kb1_hardening_tail.py`, `test_chat_proxy.py`, `test_chat_templates.py`, `TestClient`, `test_slots_image_pull.py`, `create_slot`, `test_v1_npu_trio_routing.py`, `test_exposure.py`, `test_settings_models_store.py`, `test_cli_auth_streamtest.py`, `test_models_routes.py`, `test_chat_proxy_auth.py`, `test_models_preview.py`, `auth.py`, `FastAPI`, `test_tts_request_defaults.py`, `test_typed_errors.py`, `conftest.py`, `test_profiles_crud.py`, `test_typed_bodies.py`, `test_install_apply.py`, `test_models_add_from_path.py`, `test_models_default.py`, `TestClient`, `test_v1_chat_slot_alias.py`, `test_v1_slot_alias_models.py`, `FakeWsServer`, `test_event_contract.py`, `test_redact.py`, `MemoryConfig`, `test_pull_routes.py`, `_StubSlotManager`, `BootReport`, `PgVectorProvider`, `conftest.py`, `test_installer_routes.py`, `test_comfyui_fetch_route.py`, `test_pull_shutdown.py`, `_lxc_dri`, `test_slots_container_state.py`, `TestClient`, `TestClient`, `test_auth_rotate.py`, `migrate_slot_toml`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `SlotManager` connect `SlotManager` to `test_flm_container_spec.py`, `Slot`, `SlotState`, `Dispatcher`, `build_auto_selections`, `TestLoopbackFenceCommand`, `ContainerProvider`, `test_model_fallback.py`, `FakeManager`, `FakeContainerProvider`, `test_model_preferred_profile.py`, `_write_slot`, `GpuArbiter`, `test_device_profile_coherence.py`, `test_load_converges_config.py`, `compute_config_drift`, `FakeSlotManager`, `_write_slot`, `test_npu_trio_reconcile.py`, `manager.py`, `test_npu_exclusivity.py`, `SlotIdentityStore`, `_RecordingSlotManager`, `heal_missing_llm_type`, `test_slot_aliases.py`, `RoutingHost`, `test_slot_create_conflict.py`, `test_transition_guard.py`, `test_adopted_slot_eviction.py`, `test_pulling_serving_idle.py`, `test_schema_seeds_c5.py`, `test_mtp_defuse.py`, `rerender_slot_units`, `agents-overview.jsx`, `test_npu_trio_shadow.py`, `Path`, `test_manager_npu_container.py`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `SlotState` connect `_RecordingSlotManager` to `SlotInterface`, `SlotManager`, `Slot`, `SlotState`, `Dispatcher`, `test_metrics_prometheus_route.py`, `router.py`, `test_profile_derivation_parity.py`, `test_fail_watcher.py`, `ContainerProvider`, `StackApplyEngine`, `test_crash_loop_breaker.py`, `_dispatcher`, `api_get`, `test_npu_swap_status.py`, `GpuArbiter`, `flm_served_models`, `StackConfig`, `test_backends_npu_canonical_fields.py`, `test_fail_watcher_warming.py`, `fetch_npu_swap_status`, `Any`, `arbiter.py`, `_ArbiterSlotManager`, `config_enrichment`, `Any`, `_FakeSM`, `test_npu_occupancy.py`, `_safe_query`, `proxy_board_events`, `_slot`, `container_enrichment`, `SlotStateRecord`, `SlotWatchdog`, `write_toml_atomic`?**
