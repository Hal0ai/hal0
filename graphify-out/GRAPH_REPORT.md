...
## Corpus Check
- 1622 files · ~2,051,636 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29381 nodes · 55852 edges · 1342 communities (1209 shown, 133 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 6869 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2fd6abe2`
...
## Communities (1342 total, 133 thin omitted)

### Community 0 - "dashboard-redesign.jsx"
Cohesion: 0.02
Nodes (136): Hal0Error, ADR-0004, ADR-0013, ADR-0023, useActivityRecent(), CELL_DEFS, CELL_MAP, CellDef (+128 more)

### Community 1 - "useSlots.ts"
Cohesion: 0.03
Nodes (137): ADR-0022, ChatTemplate, useChatTemplates(), useMetaEnums(), useModels(), usePullJob(), deviceBackend(), DEFAULT_METRICS (+129 more)

### Community 2 - "Slot"
Cohesion: 0.03
Nodes (83): container_provider(), Return the process-wide ContainerProvider singleton., flm_id_to_tag(), Resolve a hal0 ``<tag>-FLM`` id back to FLM's native ``family:size`` tag.      I, Atomically write a slot TOML.      THE byte-level write path for ``/etc/hal0/slo, Hold the coarse cross-process lock for ALL slots/*.toml writes.      Historicall, slot_write_lock(), write_slot_toml() (+75 more)
...
Cohesion: 0.03
Nodes (104): _fields_for_dataclass(), _fields_for_entry(), _filters_to_config(), _filters_to_runtime(), Any, AsyncClient, Lock, Path (+96 more)

### Community 5 - "stacks.jsx"
Cohesion: 0.03
Nodes (116): api(), useMeta(), useAddModelFromPath(), useModelDuplicate(), useModelSetDefault(), useModelUpdate(), useScanPreview(), ProfileBody (+108 more)

### Community 6 - "SqliteModelRegistry"
Cohesion: 0.03
Nodes (85): _import_into(), import_toml_to_sqlite(), ImportReport, _load_toml_models(), Connection, Model, Path, One-shot, idempotent import: ``registry.toml`` → the SQLite ``model`` table.  Tw (+77 more)
...
Cohesion: 0.04
Nodes (57): ProfileConfig, One ``[profile.<name>]`` entry in profiles.toml.      A profile is a reusable ba, Return the full flag string for *profile*, expanding MTP when set.      When the, resolve_profile_flags(), Classify a profile's runtime family from its TYPED fields — the profile     ``na, _runtime_family(), test_vulkan_profile_drafts_on_vulkan_device(), §7.1a / ML-5: profile.mtp is informational only — resolve_profile_flags (+49 more)

### Community 9 - "ConfigParseError"
Cohesion: 0.03
Nodes (87): config_validate(), Validate all config files against the current schema., ConfigError, ConfigNotFound, ConfigParseError, _drop_dead_keys(), _find_manifest_path(), _flatten_slot_toml() (+79 more)
...
Cohesion: 0.04
Nodes (83): The model registry could not be consulted (outage, not a miss).      Raised by t, RegistryUnavailableError, admit(), CandidateSlot, _estimate_incoming_footprint_mb(), EvictionStep, _gather_candidates(), PreloadEvictHost (+75 more)

### Community 12 - "Model"
Cohesion: 0.04
Nodes (105): NamedTuple, FileSetEntry, FileSetPlan, One file of a planned file-set, ready for ``model_file`` insertion., The full set of files one model pull should download + register., Model, A model entry in the hal0 registry.      All fields are optional at construction, _comfyui_models_dir() (+97 more)

### Community 13 - "ENDPOINTS"
Cohesion: 0.04
Nodes (100): apiDelete(), apiGet(), ApiOptions, apiPut(), serialiseBody(), ENDPOINTS, AgentMCPClientList, AgentMCPClientView (+92 more)

### Community 14 - "Any"
Cohesion: 0.03
Nodes (82): _artefact_token(), _best_effort_model_info(), _binary_runner(), _container_runtime(), _effective_backend_and_device_class(), _effective_parallel(), _effective_runner(), _llama_argv_segments() (+74 more)

### Community 15 - "useModels.ts"
Cohesion: 0.04
Nodes (92): useApproveApproval(), useDenyApproval(), useConfigUrls(), useMemoryEnabled(), AddFromPathRequest, fmtBytes(), fmtEta(), fmtSpeed() (+84 more)

### Community 16 - "die"
Cohesion: 0.04
Nodes (105): agent_install(), agent_list(), agent_peers(), approvals_approve(), approvals_deny(), approvals_list(), List installed bundled agents., List pending agent approval requests. (+97 more)
...
Cohesion: 0.04
Nodes (97): Die early on an editable/dev install — `hal0 update` can't swap the FHS tree., _refuse_if_editable(), Tag-based release-policy derivation — the single source of truth for classifying, Raised when a tag does not match any supported release pattern., Immutable policy derived from a parsed release tag., ReleasePolicy, ReleaseTagError, _atomic_symlink_swap() (+89 more)

### Community 19 - "run_pull"
Cohesion: 0.05
Nodes (99): hf_download_url(), make_job(), persist_pull_job(), pull_job_file(), Build the canonical HuggingFace download URL.      ``resolve/main`` (not ``raw/m, Create a fresh job record for ``model_id``., Path of the durable snapshot for ``model_id``'s pull job., Atomically mirror a pull-job snapshot to disk (best-effort, fail-soft).      A f (+91 more)
...
Cohesion: 0.05
Nodes (90): backfill_coordless(), CandidateModel, find_candidates(), _is_mmproj_sidecar(), _is_skippable(), _match_curated(), _maybe_register_shard_files(), _normalise_id() (+82 more)

### Community 25 - "Updater"
Cohesion: 0.06
Nodes (91): _current_symlink(), Return ``<usr_lib>/hal0-<version>/`` — where this release's tree lives., Return ``<usr_lib>/current`` — the atomic-swap target., Atomic self-update with cosign-verified releases and one-step rollback.      All, Initialise the updater.          Args:             channel: Release channel — "s, Updater, _versioned_install_dir(), _build_release_tarball() (+83 more)

### Community 26 - "ContainerProvider"
Cohesion: 0.04
Nodes (63): ContainerProvider, Podman-container-per-slot inference backend.      One instance is shared across, Not applicable — systemd starts the container., Probe GET /health on the container port.          For llama-server slots /health, Poll /health until 200 or HEALTH_TIMEOUT_S exceeded.          Raises:, Async generator that runs ``<runtime> pull <image>`` and yields         layer-pr, _make_provider_with_tmp_unit(), Path (+55 more)
...
Cohesion: 0.05
Nodes (78): _compact_ts_to_iso(), _import_v1_index(), Convert generate_results_json.py's index.json records to schema-2., One server_ab timed run → a schema-2 Rep. Returns None for an errored run     (n, server-ab stamps are compact UTC (``20260704T212506Z``); normalise to the     IS, Normalize a v1 timestamp to a run_id (UTC stamp + suffix). Keeps chrono     sort, _sa_generative_record(), _sa_latency_record() (+70 more)
...
Cohesion: 0.03
Nodes (37): LiveAgentCard(), RESTART_LABELS, useFlip(), useTilt(), AGENT_TAB_DEFS, _agentRoute(), _agentTabs(), AgentView() (+29 more)

### Community 39 - "__init__.py"
Cohesion: 0.05
Nodes (77): AbstractEventLoop, _auto_resume_interrupted_pulls(), _boot_audit_store(), _boot_background_tasks(), _boot_capabilities(), _boot_dispatcher(), _boot_mcp_memory_call(), _boot_memory_dispatch() (+69 more)

### Community 40 - "ModelRegistry"
Cohesion: 0.05
Nodes (37): ModelCapabilities, Model — registry entry pydantic model.  The Model class is the typed representat, # NOTE: revisit in Phase 1 — extend as providers surface new capabilities., Launch/runtime typed flags — §7.1d / ML-6.      Distinct from :attr:`Model.capab, Tests for ``ModelCapabilities`` + the ``Model`` §7.1d field additions.  Covers:, test_model_capabilities_defaults_all_none(), test_model_capabilities_forbids_extra_fields(), test_model_capabilities_tri_state() (+29 more)
...
Cohesion: 0.04
Nodes (34): ImageGenConfig, Any, [image] table in a slot TOML — persisted image-gen settings (#599).      Carried, Pydantic model for a single slot's TOML config (slots/<name>.toml).      Fields, Pull a `[server]` TOML table out of the loader's `extra` catch-all.          ``h, Pull a `[npu]` TOML table out of the loader's `extra` catch-all.          Mirror, Pull an `[image]` TOML table out of the loader's `extra` catch-all.          Mir, Read-only promotion shim: derive ``device`` from a legacy         on-disk ``back (+26 more)
...
Cohesion: 0.06
Nodes (65): _cache_slot_enrichment(), _config_field_enrichment(), _container_state_enrichment(), delete_slot(), get_slot(), get_slot_by_id(), get_slot_by_name(), get_slot_config() (+57 more)
...
Cohesion: 0.05
Nodes (35): load_profiles_config(), Load and validate /etc/hal0/profiles.toml.      Returns a :class:`ProfilesConfig, Strip hal0-managed flags (§21.7) from non-seed profiles, healing disk.      Olde, Look up a named profile in the profiles.toml catalog.      Shared by every provi, resolve_profile(), _sanitize_custom_profile_flags(), ProfilesConfig, Parsed profiles.toml — top-level ``[profile]`` table.      Each key under ``[pro (+27 more)
...
Cohesion: 0.06
Nodes (59): _fetchServers(), McpServer, McpServerActivity, McpServerState, McpTool, MOCK_SERVERS, useMcpServers(), CatalogEntry (+51 more)
...
Cohesion: 0.05
Nodes (54): _build_offline_deps(), Construct a SlotManager + model registry WITHOUT a running API, mirroring     ho, EventBus, make_event(), _now_iso(), Any, Queue, In-process event bus + ring buffer for the dashboard footer.  The footer status (+46 more)
...
### Community 73 - "Path"
Cohesion: 0.05
Nodes (64): _apply_config_set(), _build_config_overlay(), _copy_plugin_tree(), _copy_wrapper(), _dirs_identical(), _disable_honcho_hermes_host(), _ensure_hermes_config(), _hal0_service_home() (+56 more)
...
Cohesion: 0.05
Nodes (62): _active_persona_render(), BootstrapState, _brain_profile_config_path(), _default_mcp_servers(), _fetch_model_contexts(), _generate_api_server_key(), hermes_pinned_ref(), hermes_requirement_floor() (+54 more)

### Community 81 - "Any"
Cohesion: 0.05
Nodes (63): _build_brain_identity_card(), _build_identity_card(), _collect_capability_rollup(), _collect_chat_slots(), _fetch_slots(), _find_named_ready_slot(), _find_slot(), _fmt_config_value() (+55 more)
...
### Community 101 - "v1.py"
Cohesion: 0.09
Nodes (53): DispatcherDep, _aggregate_models(), audio_speech(), audio_transcriptions(), _capability_slot_for_path(), chat_completions(), completions(), _dispatch_and_forward() (+45 more)
...
Cohesion: 0.07
Nodes (52): build_parser(), cmd_devices(), cmd_eval(), cmd_history(), cmd_import_v1(), cmd_plan(), cmd_publish(), cmd_reindex() (+44 more)
...
### Community 110 - "test"
Cohesion: 0.04
Nodes (13): test, MOBILE, CURATED, ROWS, UPSTREAM_ROW, NPU_SERVING_SLOT, SECTIONS, ADR-0012 (+5 more)
...
Cohesion: 0.07
Nodes (39): _flatten_for_ui(), Project HardwareInfo into the fields the Vue Hardware view expects.      The das, load_hardware_info(), Load /etc/hal0/hardware.json and return a validated HardwareInfo.      Returns t, Atomically write hardware.json.      Uses the same tmpfile+fsync+rename pattern, save_hardware_info(), HardwareInfo, NPUInfo (+31 more)
...
Cohesion: 0.07
Nodes (50): add_model_from_path(), check_model_updates(), create_model(), delete_model(), delete_pull(), duplicate_model(), get_model(), inspect_model() (+42 more)
...
Cohesion: 0.07
Nodes (34): ChangeSet, FileState, fold_ctx_size_alias(), _known_field_names(), Any, Path, SlotConfigStore — one reconciled truth for capability + slot config (issue #697), Fold the legacy ``[model].ctx_size`` alias into the canonical     ``context_size (+26 more)
...
Cohesion: 0.09
Nodes (44): active_tools_for(), Return the filtered tool list for a chat slot, per plan §7.3.      Args:, FakeSlotManager, _model_id_of(), Any, Shared fixtures for the OmniRouter test suite.  The OmniRouter only talks to two, Tiny stub that satisfies :class:`SlotManagerLike`.      Tests build it with a li, _caller_no_tools_label() (+36 more)
...
Cohesion: 0.06
Nodes (42): create_app(), Lifespan wiring for the GpuArbiter idle-restore loop (Phase D, Task D6).  The ap, test_lifespan_starts_and_cancels_arbiter_idle_loop(), MonkeyPatch, TestClient, A failed MCP mount must be visible, not just logged and forgotten.  The fastapi-, A mount failure must flip /api/health/system to degraded and say why., create_app must stash the reason, not only log it. (+34 more)
...
Cohesion: 0.07
Nodes (31): Fire-and-forget trigger to refresh the Hermes live-context files.  Called from t, Spawn a detached ``hal0-agent <agent_id> render-context``. Never raises., spawn_context_refresh(), CapabilityApplyFailed, CapabilityOrchestrator, legal_children(), Any, CapabilitySelection (+23 more)
...
Cohesion: 0.08
Nodes (46): Cell, One planned unit of work: a fully-resolved identity + the suite context     the, _assemble(), _classify(), _clear_stale_sweep(), describe_worklist(), fetch_host(), _friendly_gpu() (+38 more)
...
Cohesion: 0.08
Nodes (46): npu_trio_slot_root(), TestClient, coresident_group must key off device==npu, not the legacy slot names.      Deplo, Lay down the NPU FLM trio (agent + stt-npu + embed-npu) on disk., A model-less NPU LLM anchor → no trio markers on the siblings (#1369).      This, #1369: membership follows the anchor, with no per-sibling opt-out.      The shad, A slot's [model].n_gpu_layers rides along in the payload., spec-hw-slot-ownership §1: a stray slot-level ``enable_thinking`` (pre-     migr (+38 more)
...
Cohesion: 0.07
Nodes (26): Typed launch plan for a container-per-slot Quadlet unit.      Carries everything, RuntimeLaunchPlan, _loopback_fence_command(), Flip any ``0.0.0.0`` bind in *command* to loopback (host-net fence).      THE si, Render a Podman Quadlet ``.container`` unit from a launch plan.      The ONE ren, _render_quadlet_from_plan(), _exec_tokens(), halo150/143 O8+O11: ONE render for every substrate — no version branch.      Nat (+18 more)

### Community 160 - "RoutingHost"
Cohesion: 0.08
Nodes (33): See :func:`hal0.slots.routing.loaded_slot_from_config`., See :func:`hal0.slots.routing.loaded_slot`., See :func:`hal0.slots.routing.resolve_for_request`., add_slot(), default_slot_for(), loaded_slot(), loaded_slot_from_config(), LoadedSlot (+25 more)

### Community 161 - "agent_commands.py"
Cohesion: 0.06
Nodes (44): _detect_foreign_gateways(), Retired foreign-gateway scan — now a no-op shim.      hal0's single-managed-gate, agent_log(), _agent_provision_state_file(), agent_reprovision(), agent_status(), _approval_summary(), bootstrap_hermes() (+36 more)
...
### Community 165 - "json"
Cohesion: 0.06
Nodes (19): json(), hermes(), IDLE_PRIMARY, mockAgents(), AuthOpts, installAuth(), LIVE_BOARD, HERMES_ACTORS (+11 more)
...
Cohesion: 0.08
Nodes (24): Build the GPU/llama-server :class:`RuntimeLaunchPlan` for a slot.          This, _device_node_for_group(), gpu_visibility_env(), is_nvidia_gpu_device(), nvidia_cdi_devices(), _probed_gpu_group_gids(), Shared helpers for GPU device + group exposure to provider containers.  Lives he, Return numeric GIDs for the host's GPU access groups (render, video).      ``nod (+16 more)
...
Cohesion: 0.07
Nodes (18): emitSseTyped(), emitWs(), installWsHarness(), waitForWs(), NOTE: addInitScript serialises the function — it must not close over, body(), engine(), hero() (+10 more)
...
Cohesion: 0.09
Nodes (33): agent_uninstall(), Uninstall a bundled agent., Best-effort: delete the hermes identity card from the `agents` dataset.      Fai, _uninstall_hermes_memory(), graph_disable_cmd(), Turn graph extraction OFF; cancels any in-flight build., _api_reachable(), api_delete() (+25 more)

### Community 261 - "ModelsConfig"
Cohesion: 0.08
Nodes (16): ModelsConfig, [models] section of hal0.toml — discovery + auto-detect., Empty means "env var / FLM default cache"; non-empty must be absolute., Roots the discovery scan actually walks: declared ``roots`` plus the         eff, Reject relative paths — discovery walks must start from an absolute root., Empty means "use pull_root"; non-empty must be absolute., Return the resolved model-store path consumers should point at.          Precede, MonkeyPatch (+8 more)
...
### Community 304 - "Table"
Cohesion: 0.09
Nodes (28): mm_history_cmd(), mm_list_cmd(), mm_refresh_cmd(), ``hal0 memory mm`` — mental-model admin (list / refresh / history)., List a bank's mental models., Trigger a refresh of a mental model (async)., Show a mental model's revision history., _require_api() (+20 more)
...
Cohesion: 0.11
Nodes (27): client_factory(), fake_container(), _hermetic_port_listeners(), make_create_body(), AbstractContextManager, MonkeyPatch, TestClient, Shared fixtures for the golden-path harness.  The whole harness drives the PUBLI (+19 more)
...
Cohesion: 0.08
Nodes (25): Return the TTS slot profile a device selection implies.      The engine switch:, tts_profile_for_device(), profile_name_for_fit(), Shared picker/apply profile-fit inference (device → runtime profile name).  Sing, Infer the runtime profile name implied by a picker/apply selection.      Keeps i, Any, voice.tts capability switch — Kokoro (CPU) vs Qwen3-TTS (GPU/ROCm).  The deferre, Write the canonical single ``tts`` slot in a known engine state. (+17 more)
...
Cohesion: 0.10
Nodes (28): _dev_mode(), _fetch_slot_drift(), _interactive(), _poll_job(), _print_check(), _print_drift_banner(), Context, Path (+20 more)
...
### Community 355 - "Enum"
Cohesion: 0.11
Nodes (25): Enum, auth_require(), auth_rotate(), auth_status(), KeyTier, hal0 auth subcommands — thin HTTP client to /api/auth/*.  §5.2 of the R5 sync as, Persist the [security].require_auth posture (PUT /api/auth/require)., Show the current auth posture (GET /api/auth/status). (+17 more)
...
### Community 364 - "Path"
Cohesion: 0.07
Nodes (27): Path, Regression: the slot-edit Save's ``PATCH /defaults`` must not wipe the     model, #585: the dashboard writes ``ctx_size``; it must persist as the canonical     ``, PR-18: each entry carries ``type`` + ``model_default``.      The dashboard's per, Labels list is omitted (not empty) when the slot config has no     ``model.label, A failing container health probe doesn't break /api/slots.      The enrichment s, device=npu slots surface declared_backend='flm' (the FLM recipe)., POST /api/slots/chat/load goes through ContainerProvider.load_sync. (+19 more)
...
Cohesion: 0.08
Nodes (20): mock_slot_manager(), Path, Tests for the slot back-compat alias system (issue #654 / #633, ADR-0023).  ADR-, _resolve_alias("agent-hermes") == "agent" and the agent TOML exists., list() enumerates TOMLs from disk — no alias appears in the result., iter_configs() is driven by disk TOMLs — aliases never appear., No alias map; hal0/primary is not a known virtual., ADR-0023: ``utility`` is seeded; ``chat``/``primary`` are retired. (+12 more)
...
Cohesion: 0.17
Nodes (21): Pull the latest matching ``hermes-agent`` into the venv + reconcile config., upgrade_hermes_runtime(), agent_upgrade(), Upgrade Hermes to the latest matching release, then reprovision.      Pulls the, _fake_venv(), FakeRunner, _is_migrate(), _is_pip() (+13 more)
...
### Community 436 - "Any"
Cohesion: 0.08
Nodes (23): container_stub(), Any, GET /api/slots/{name} is enriched same shape as the list endpoint., The enriched body must be valid JSON (no exotic types leaked)., An explicit upstreams.toml entry for ``hal0`` survives startup unchanged.      O, Loading a slot with no TOML returns the typed slot.not_found envelope., Verifies the /unload lifecycle route reaches the SlotManager., PUT /config has no lifecycle side effect — a running slot stays running.      Th (+15 more)
...
Cohesion: 0.12
Nodes (18): Any, Shared config-echo redaction (issue #553).  A single helper that scrubs sensitiv, Recursively scrub sensitive-keyed values from a config tree.      - ``dict``: wa, Project a sensitive *value* into the masked ``{value, set}`` shape.      ``set``, redact_config(), redact_value(), A non-sensitive key (e.g. ``ctx_size``) is echoed verbatim., An empty sensitive value is masked with ``set=false`` so the UI can     render t (+10 more)

### Community 538 - "_slot_to_dict"
Cohesion: 0.14
Nodes (19): load_slot(), Slot, ValidationError, Load a model into a slot. Optional body: {"model_id": "..."}.      Validates ``m, Unload a slot. A pinned slot (§21.10) requires ``?force=true``.      Pass ``?for, Serialise a real Slot snapshot into the API shape.      Request-bound adapter ov, Hot-swap a slot's model. Body: {"model_id": "..."}.      The swap path is destru, Render a pydantic ValidationError into ``{field_path: message}``.      Same shap (+11 more)
...
Cohesion: 0.11
Nodes (17): Tests for hal0.api._redact — shared config-echo redaction (#553).  Every config-, Acceptance criterion #1: a known token-bearing key comes back masked,     with `, Nested dicts are scrubbed: any sensitive key at any depth is masked., A sensitive *container* key over-redacts its whole value (by design).      ``re., A list of scalars is not masked — only keyed containers are scrubbed., If an Upstream entry carries an api_key in its extra-allow dict, the     seriali, An empty string is treated as 'unset' so the UI can render a blank slot., A None value is treated as 'unset'. (+9 more)
...
Cohesion: 0.15
Nodes (7): probe_slot_name(), Any, Shared helper for ContainerProvider probe doubles (#1417).  Production hands the, The slot name a probe argument refers to (config mapping or bare token)., FakeContainerProvider, Any, In-memory ContainerProvider double mirroring the surface SlotManager     touches
...
Cohesion: 0.21
Nodes (16): _patch_httpx(), MonkeyPatch, Tests for the /api/slots route surface (container runtime).  Covers:   - list-me, n_prompt_tokens can briefly exceed n_ctx during shift; clamp the     synthesised, v0.1.x clients consuming /api/slots see every legacy key unchanged., Minimal httpx.Response stand-in for the scrape tests.      Implements just the s, Patch httpx.AsyncClient used inside slots._scrape_llama_metrics.      Routes the, Newer llama-server (b9279+) drops kv_cache_usage_ratio from     /metrics but sti (+8 more)
...
Cohesion: 0.20
Nodes (15): _layout_path(), load(), Any, Path, Persistent file-backed store for the operator's dashboard layout.  Single-operat, Write *layout* to disk atomically.      Callers should call :func:`reconcile` be, Return a defensively-normalised copy of *layout*, per its own version.      A v3, Return a defensively-normalised copy of a legacy v2 *layout*.      Rules applied (+7 more)
...
Cohesion: 0.23
Nodes (15): app_isolated(), hanging_run_pull(), Any, FastAPI, Issue #1225: `systemctl restart hal0-api` must not hang on a live model pull.  C, Shutdown must cancel a still-running pull and finish promptly — the     core of, No in-flight pulls → shutdown doesn't wait on anything (regression     guard: th, A client (re)subscribing to the SSE progress stream while hal0-api is     shutti (+7 more)
...
Cohesion: 0.16
Nodes (14): _DuplicateModelBody, ``POST /scan/preview`` body — only ``paths`` needs real typing.      ``recursive, ``POST /{model_id}/default`` body.      ``default`` is typed ``Any`` ON PURPOSE,, ``POST /{model_id}/duplicate`` body., _ScanPreviewBody, _SetModelDefaultBody, _build_curated(), HaloaiModel (+6 more)
...
Cohesion: 0.15
Nodes (14): _atomic_write(), _atomic_write_if_changed(), _build_brain_profile_mcp_servers(), content_hash(), _deep_merge(), _merge_config_yaml_layers(), _phase_brain_profile_mcp_wire(), Stable content hash steps use to detect "inputs unchanged".      Steps that prod (+6 more)
...
Cohesion: 0.25
Nodes (8): hal0_chat_slot_alias_map(), Return ``{slot_alias: model_id}`` for model-bound llm slots.      The slot **ali, hal0_chat_slot_alias_map returns the canonical agent slot's model_id., A utility-only slot set carries `utility` but injects no extra alias     (agent-, If a literal 'agent-hermes' slot still exists on disk, it takes precedence., test_chat_slot_alias_map_alias_does_not_override_explicit(), test_chat_slot_alias_map_includes_agent_canonical(), test_chat_slot_alias_map_utility_no_alias_injection()
...
Cohesion: 0.33
Nodes (4): When the NPU LLM anchor has a model bound, every trio slot gets coresident_group, #863: single-slot status includes argv drift without burdening list()., test_get_slot_includes_config_drift_when_requested(), test_list_slots_emits_coresident_group_for_npu_trio()
...
## Knowledge Gaps
- **3304 isolated node(s):** `runlog.sh script`, `rocmfpx-build.sh script`, `rocmfpx-env.sh script`, `rocmfpx-pipeline.sh script`, `rocmfpx-quantize.sh script` (+3299 more)
...
- **Why does `create_app()` connect `create_app` to `test_pull_shutdown.py`, `TestClient`, `FastAPI`, `test_hf_routes.py`, `test_stacks_routes.py`, `_RecordingSlotManager`, `conftest.py`, `ApprovalQueue`, `isolated_client_with_root`, `run_pull`, `test_route_collisions.py`, `MemoryConfig`, `test_auth_core.py`, `test_memory_admin_routes.py`, `test_slots_policy.py`, `test_dashboard_layout.py`, `test_settings_routes.py`, `test_memory_delete_auth.py`, `__init__.py`, `test_models_crud.py`, `test_updater_routes.py`, `test_admin_route_map_nested.py`, `test_config_model_roots.py`, `Any`, `test_kb1_hardening_tail.py`, `mcp_mount.py`, `TestClient`, `test_slots_image_pull.py`, `test_chat_proxy.py`, `test_models_routes.py`, `test_v1_npu_trio_routing.py`, `EventBus`, `_build`, `test_models_preview.py`, `test_dashboard_layout_v3.py`, `test_settings_models_store.py`, `test_exposure.py`, `hal0_slot_alias_models`, `test_chat_proxy_auth.py`, `test_tts_request_defaults.py`, `test_route_sync.py`, `auth.py`, `conftest.py`, `load_hal0_config`, `test_profiles_crud.py`, `test_typed_errors.py`, `test_typed_bodies.py`, `FakeWsServer`, `TestClient`, `test_startup_slot_seed.py`, `test_chat_templates.py`, `types.py`, `_StubSlotManager`, `test_models_add_from_path.py`, `test_models_default.py`, `test_pull_routes.py`, `test_cli_auth_streamtest.py`, `PgVectorProvider`, `test_event_contract.py`, `conftest.py`, `test_comfyui_fetch_route.py`, `test_events_fixes.py`, `isolated_client`, `test_v1_chat_slot_alias.py`, `test_installer_routes.py`, `BootReport`, `test_auth_rotate.py`, `migrate_slot_toml`, `test_slots_container_state.py`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `SlotManager` connect `SlotManager` to `Slot`, `test_gpu_arbiter.py`, `SlotConfigError`, `FLMProvider`, `SlotInterface`, `Dispatcher`, `test_model_preferred_profile.py`, `RoutingHost`, `_write_slot`, `test_load_converges_config.py`, `test_device_profile_coherence.py`, `test_model_fallback.py`, `test_mtp_defuse.py`, `GpuArbiter`, `FakeSlotManager`, `compute_config_drift`, `SlotWatchdog`, `_write_slot`, `manager.py`, `test_npu_trio_reconcile.py`, `EventBus`, `heal_missing_llm_type`, `FakeContainerProvider`, `SlotState`, `test_slot_create_conflict.py`, `test_npu_exclusivity.py`, `test_transition_guard.py`, `test_adopted_slot_eviction.py`, `SlotIdentityStore`, `SlotStateRecord`, `v1.py`, `test_pulling_serving_idle.py`, `agents-overview.jsx`, `test_npu_trio_shadow.py`, `rerender_slot_units`, `Path`, `test_slot_aliases.py`, `test_manager_npu_container.py`, `.__init__`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `connect()` connect `connect` to `_safe_query`, `PortAuthority`, `gc.py`, `test_pull_refcount_rows.py`, `SqliteModelRegistry`, `Path`, `useActivity.ts`, `Model`, `test_stats_requests_route.py`, `useLogs.ts`, `hw_slot_ownership.py`, `MetricsWriter`, `NotFound`, `test_discover.py`, `SlotIdentityStore`, `ModelDefaults`, `aggregate_hour`?**
