...
All provisioning endpoints (POST /apply, POST /apply-selections, POST /complete)
are idempotent and can be re-run at any time. ``GET /state`` stays readable
forever so the dashboard can always confirm ``first_run: false``.
"""
...
async def install_complete(request: Request) -> dict[str, Any]:
    ...
    write can't leave a half-written marker.  Idempotent — re-running
    after setup overwrites the sentinel cleanly.
    """
    sentinel = _first_run_sentinel()
