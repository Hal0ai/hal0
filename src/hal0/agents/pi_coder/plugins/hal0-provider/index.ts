...
// Bundled agent runs on the same box as the gateway (ADR-0004 §4);
// HAL0_API_URL overrides for remote use.
...
async function fetchHal0Models(): Promise<Model<"openai-completions">[]> {
  ...
  for (const entry of body.data) {
    ...
    const ctx = entry.max_context_window ?? entry.context_length ?? 128_000;
    models.push({
      id: entry.id,
      name: displayName,
      provider: "hal0",
      ...
      reasoning: false,
      input: entry.labels?.includes("vision") ? ["text", "image"] : ["text"],
...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // fresh.length === 0 → keep last-known list (gateway down or warming).
  });
}
