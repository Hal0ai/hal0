...
#     bin        = "/usr/local/bin/hermes"     # default: per-type
#     port       = 9119                        # default: per-type (hermes 9119)
#     host       = "127.0.0.1"                 # default: loopback only
#     serve_args = ["--port", "9119", ...]     # exact server flags,
...
# Per-type default loopback port. hermes → upstream's 9119.
_DEFAULT_PORTS: dict[str, int] = {"hermes": 9119}
...
class AgentConfig:
    ...
    host: str
    port: int
    # Explicit binary override from the agent toml. ``None`` →
    # resolve per-type via :attr:`bin`.
    bin_override: Path | None = None
    # Exact serve-time args from the agent toml (server flags). Empty
    ...
    def bin(self) -> Path:
        ...
        hermes → the venv console script. An explicit ``bin = ...`` in
        the agent toml wins.
        """
...
def _load_agent_config(agent_id: str) -> AgentConfig:
    ...
    # is per-type (hermes 9119), 9119 for anything unknown.
