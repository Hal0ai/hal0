...
# converges: SlotManager._await_ready polls /health for up to
# providers.container._HEALTH_TIMEOUT_S (180s) per load. ``restart`` runs
# unload (terminate, bounded at SlotManager._terminate_timeout_s = 30s)
# followed by a full load, so its worst-case budget is ~210s. api_post's
# default 10s read timeout is far under either budget — every mutating
# lifecycle verb would ReadTimeout and exit 1 on an operation that in fact
# succeeded server-side (#1832). Give these call sites a client timeout with
# headroom over the server's own worst case instead of the blanket default.
