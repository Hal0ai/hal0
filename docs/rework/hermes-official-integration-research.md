...
2. **hal0 is pinned to a materially older fork and contract.** `pyproject.toml` currently records `earendil-works/hermes-agent@0554ef1` dated 2026-05-28, while the official repository is `NousResearch/hermes-agent` and current main is `7fd419e`. Therefore every item below needs an upstream-pin migration/compatibility phase before implementation.
3. **The existing `hal0-memory` idea remains architecturally correct, but its loader contract must be retested.** Official Hermes now has a formal `MemoryProvider` lifecycle, a one-active-external-provider rule, setup schema, profile/session identity inputs, per-turn prefetch/sync, compression and delegation hooks, tools, and plugin CLI discovery ([ABC source](https://github.com/NousResearch/hermes-agent/blob/7fd419e5e6a0ac53f934a69226262c41ba130a2c/agent/memory_provider.py), [authoring guide](https://github.com/NousResearch/hermes-agent/blob/7fd419e5e6a0ac53f934a69226262c41ba130a2c/website/docs/developer-guide/memory-provider-plugin.md)).
...
- Assume every loaded Python plugin has Hermes-process authority. Restrict what secrets Hermes receives, sandbox the service, keep plugin source root-owned/read-only, and expose privileged hal0 mutations only through authenticated, audited APIs ([official security boundary](https://github.com/NousResearch/hermes-agent/security)).
...
1. Change the source-of-record repository from the stale fork URL only after verifying lineage and the desired official tag/commit.
...
1. **Update the upstream record.** The current `earendil-works` pin is not the current official source. Preserve it as historical provenance, but plan migration against `NousResearch/hermes-agent`.
...
3. **Retest `hal0-memory` discovery.** The local comment claims `$HERMES_HOME/plugins/<name>` discovery with `register(ctx)`, while official documentation places memory providers in `$HERMES_HOME/plugins/memory/<name>` with specialized discovery. Treat this as a probable compatibility bug until tested against the chosen pin ([official layout](https://github.com/NousResearch/hermes-agent/blob/7fd419e5e6a0ac53f934a69226262c41ba130a2c/website/docs/developer-guide/memory-provider-plugin.md#directory-structure)).
