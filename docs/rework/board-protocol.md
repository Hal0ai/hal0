...
> **Where the live state lives:** this in-repo file is the durable *process* reference (stable). The
> *live* orchestration state — `REWORK.md` (spec), `REWORK_BOARD.md` (board), `hal0-board.sh` /
> `hal0-status.sh` (monitor) — lives in the orchestration workspace at `/home/mint/` (out of tree, it
> changes every session). This copy travels with the code so the protocol survives a fresh checkout.
