...
| hostnet-render | operator-validated 143 fix: paired `Network=host` + Exec bind `--host 127.0.0.1` (bind = the LAN fence), config-driven substrate default (NO runtime probing) + same-version venv refresh gate (commit-stamp, not version string) | 🔨 | SLOT/DEPLOY | podman-unprivileged-findings✔ | lane `rework/hostnet-render` (Opus, in flight) | chokepoint invariant: host-net plan can never render 0.0.0.0 bind; bridge regression pinned | unblocks 143 slot-load; venv gate kills the same-version stale-code trap |
