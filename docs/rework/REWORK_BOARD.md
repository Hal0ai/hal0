...
| HP-realtime | inc-1 IN FLIGHT (promoted from post-core, user 2026-07-19): `WS /v1/realtime` per spec-hp-realtime.md with THREE user decisions — (1) server VAD ships inc-1 (stock Go demo client works day one), (2) client-side MCP via the existing /mcp/admin mount made a real documented path (stdio→HTTP bridge recipe), (3) setup fully — both LLM legs (steward via /api/brain/chat SSE, generic /v1 chat + session tools), cancel, bounded approval-wait voice UX; streaming-model enablement report-only | ▶ | API/SLOT | spec e3974a78✔ | lane `rework/realtime-inc1` (Opus, in flight) | synthetic-pcm16 WS contract CI, exposure row, policy-lock entry | inc-2 remainder: streaming STT/TTS models, barge-in latency work |
