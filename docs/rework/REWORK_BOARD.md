...
| perms-recurse (O13b) | `doctor perms --fix` heals slots dir but not nested state.json — recursive coverage for slots/+registry/ trees | 🔨 | INSTALL | O13 | lane `rework/perms-recurse` (Sonnet, in flight) | nested-drift heal test + existing-row regressions | — |
| steward-nomodel | fresh-box `[brain_chat] model=""` → raw 404; emit actionable SSE error (load brain/agent slot or set model) + ≥8k-ctx docstring (system prompt ≈7.3k) | 🔨 | BRAIN | validation note | lane `rework/steward-nomodel` (Sonnet, in flight) | error-frame text tests; transport failures stay distinct | — |
...
| O20 kanban schema | hermes-gateway kanban DB missing tables (tasks, kanban_notify_subs) → watcher errors every tick, board dispatch blocked (gates Phase 5/6 tails) | ⛔ | HERMES | operator evidence pending | — | — | operator chasing: DB path, init/migrate command availability, verbatim errors → then lane the provision-side schema init |
| model-default-type | per-type model default: set = flip `default=true` + demote current holder (single-holder invariant, one chokepoint); REMOVE = unset with no promotion; slot-create `default: true` payload WIRED (was a dead checkbox — backend silently dropped it); default badge + set/remove affordance in model drawer | 🔨 | FLAGS/UI | user request 2026-07-19 | lane `rework/model-default-type` (Opus, in flight) | invariant + idempotence + create-wiring tests; exposure row if new route | consumer wiring (preselection/stamp seeding) = report-only, follow-up |
