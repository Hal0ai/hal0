...
| api-logs-redact | `/api/logs` (routes/logs.py) streams raw journalctl with ZERO redaction — secondary leak path, independent of SEC-mcp-clientid | ☐ | SEC | — | surfaced by SEC-mcp-clientid lane | Phase 1 candidate; add the same redactor to the log-stream route |
| cli-auth-streamtest | explicit auth-on smoke tier exercising every streaming verb against a keyed TestClient (§5.1 asked; CLI-auth leaned on transitive `_auth_headers` coverage) | ☐ | CLI | CLI-auth✔ | minor follow-up | Phase 1 candidate |
