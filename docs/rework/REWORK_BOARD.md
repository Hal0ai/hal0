...
| HP-realtime | OpenAI Realtime API (`wss /v1/realtime`): bidirectional WS session — interleaved audio in/out, server VAD, incremental STT, chunked TTS, tool-calling as session events (LocalAI-parity ask, user 2026-07-19) | ⏸ | API/SLOT | HP-voice (shares slot plumbing); §11.1✔ | — | protocol layer is wholly new: OpenAI event vocabulary (session.update / input_audio_buffer.* / response.*), long-lived-WS exposure+auth classification | **post-core (user)** — model-side ingredients already curated (moonshine-small-streaming-en, vibevoice-realtime-0.5b); sequence AFTER HP-voice |
