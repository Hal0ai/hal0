...
| login-ui (O19+) | dashboard login view (backend POST /api/auth/login existed since KB-1, UI never rendered it → lockout); EXPANDED per user: auth OFF by default (0.0.0.0 auto-on RETIRED — posture defeated itself), settings page must be real not stub (enable/disable auth, key status, rotate wired; enable can't lock the operator out mid-toggle) | 🔨 | UI/SEC | O19, user directive | lane `rework/login-ui` (Opus, in flight) | KB-1 tests re-pinned to new posture deliberately; e2e login specs | — |
...
| O21 virtual-id advertising | `[brain_chat] model="hal0/brain"` 404s: hal0/<slot> virtual ids NOT advertised in /v1/models and not routable on this path — only raw model ids (or "" auto-pick) work, contradicting the "any enabled slot addressable as hal0/X" claim | ☐ | API/MODEL | operator finding 2026-07-19 | — | decide: advertise hal0/* virtual models in /v1/models + make routing hold, or retire the claim; steward-nomodel lane documents current reality meanwhile | queue after model-default-type merges (routes/models.py fence) |
| O20 kanban schema | ROOT-CAUSED (operator): schema init coupled to executor registration — kanban_db.connect() auto-inits but the pinned gateway WATCHER opens the DB raw → uninitialized until HERMES_DASHBOARD_BASE_URL set (150: 0 tables vs 143: 8); init_db validated → 0 watcher errors both boxes | 🔨 | HERMES/INSTALL | operator root-cause 2026-07-19 | lane `rework/o20-kanban-init` (Sonnet, in flight) | provision-time convergent schema init via the HERMES venv's own init_db (no SQL replication = no pin drift); venv-absent → honest skip | unblocks Phase 5 liveness + Phase 6 board-card dispatch |
