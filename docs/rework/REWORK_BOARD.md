...
| steward-auth (O17/O18) | steward dead on auth-on boxes: internal /v1 POST carries no bearer (401) + brain-chat framing trips template user-query guard (500 `multi_step_tool`) | 🔨 | BRAIN | R4-stage validation | lane `rework/steward-auth` (Opus, in flight) | fix = forward caller bearer / service identity (O2 class) + template-safe message framing, wire-shape tests | MAJOR — blocks Phase 4/5/6 revalidation; 150 carries operator's local un-laned patch (leave until lane lands) |
