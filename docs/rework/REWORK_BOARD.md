...
| install-fixes (O13–O16) | fresh-install slots tree root-owned + perms --fix gap; `--adopt` flag not retired; uv-fallback HOME=/root leak (py3.14 hosts); uninstall 500 leaves HERMES_HOME | 🔨 | INSTALL/HERMES | R4-stage validation | lane `rework/install-o13-16` (Opus, in flight) | birth-ownership + PermRows, flag retirement, subprocess env hygiene, honest uninstall error surface | O16 prime suspect: rmtree-as-hal0 on root-owned entries (O13 class) |
