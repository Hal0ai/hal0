...
Ubuntu does not package `cosign`, and signature verification is mandatory
— so the bootstrap fetches a digest-pinned official cosign build into its
own temporary directory first, verifies it, uses it, and throws it away.
You do not need to install cosign yourself and nothing is left on the host.

The bootstrap then fetches the release manifest, SHA-256- and cosign-verifies
the tarball, then hands off to the installer, which installs podman, builds
the venv, writes systemd units, scaffolds the capability slots (no model
assigned to any of them except `brain`, whose steward weights it pulls),
and starts `hal0-api` on port `8080`. The installer re-execs itself under `sudo` when piped
through `bash`. See [Install hal0](/docs/getting-started/install/) for the
full sequence and every environment variable and flag.
