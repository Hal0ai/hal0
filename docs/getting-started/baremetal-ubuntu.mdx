...
the venv, writes systemd units, runs first-run setup to scaffold empty
capability slots (no model assigned to any of them), and starts `hal0-api`
on port `8080`. The installer re-execs itself under `sudo` when piped
