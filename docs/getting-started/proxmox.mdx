...
syntax). Append this to `/etc/pve/lxc/<CTID>.conf` on the host:
...
are missing or wrong — run `hal0 doctor` after passthrough to confirm.
...
unprivileged path above — an unconfined profile on a privileged container
is a wider trust boundary than you need.
