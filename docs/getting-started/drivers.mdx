...
own ROCm userspace. In particular, hal0 does **not** need `rocm-smi` to pick
the ROCm lane: `/dev/kfd` present and openable by the identity that runs
your slot containers is enough on its own. A `rocm-smi --showproductname`
that succeeds is still a second, independent signal for ROCm, so having
`rocm-smi` installed can add the lane, but its absence never removes it.
</Aside>

## Re-probing after a driver or kernel change

hal0 caches what it found in `/etc/hal0/hardware.json`, written by `hal0
config hardware --refresh`. A kernel upgrade, a newly forwarded device, or a driver change
...
shapes itself (the file is missing, carries none of the kernel/probe-time/GPU
fields, records a different kernel than the running one, or was written in
a previous boot cycle) and falls back to a live re-probe
automatically rather than trusting a stale answer — `hal0 doctor all` surfaces
