...
- `GET /api/services/health` — a stable summary over the same service
  catalog (OpenWebUI, ComfyUI, Hermes, Hindsight) that feeds the Overview
...
`up: false, state: "down", detail: "unmonitored"`, though none of the four
registered services hits that path today. The summary's probes run
concurrently, so one hung or raising probe reports its own row as `down`
