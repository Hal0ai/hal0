...
| **Turnstone** | `hal0-agent@turnstone.service` | `:9129` (loopback only) | start/stop/restart/enable/disable | Self-hosted orchestration platform — coexists alongside Hermes (relaxed single-pick). Routes all inference through hal0-api's gateway and grades each tool call with an LLM judge. No LAN URL fallback — a link is shown only when `HAL0_TURNSTONE_PUBLIC_URL` is set. |
