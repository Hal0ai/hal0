...
- **Superset org:** `thinMint's Team` · **project:** `hal0` (`1bc59c2e-5f50-40fd-9953-e540883c03e8`),
  path `/mnt/dev/repos/hal0-mono/hal0`
...
These label groups are **team-scoped**, and so exist twice: once on `HAL0`, once on `MINT`. Adding a
new one means adding it to both.
...
```sh
/mnt/dev/repos/hal0-mono/hal0/scripts/superset-linear.sh
```
...
```sh
LINEAR_ISSUE_IDENTIFIER=HAL0-2 \
LINEAR_ISSUE_BRANCH_NAME=thinmint/hal0-2-live-install-validation-on-halo150-halo143 \
...
- **The sync only covers the `MINT` team.** As of 2026-07-30 Superset mirrors 27 tasks, all
  `MINT-*`, and zero of the 12 `HAL0-*` issues — with `syncError: null` throughout, so this fails
  silently rather than loudly. Add the `HAL0` team to the Linear integration inside Superset, then
  confirm with:
  ```sh
  superset tasks list --json | jq '[.[] | select(.externalKey | startswith("HAL0"))] | length'
  ```
- **Scheduled automations do not work.** `superset automations create` fails with `Not found`
  ...
  register the host from the Superset desktop app, then re-check `superset hosts list`.
...
- **Orphaned milestones.** Five R1–R5 milestones remain on `c0zy.dev` and five more on the duplicate
  `hal0-b99627ca1118` project, created before the current project existed. There is no
  milestone-delete API — remove them in the UI.
