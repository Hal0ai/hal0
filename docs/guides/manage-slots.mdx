...
4. **`--hardware`** — the compute backend: `rocm`, `vulkan`, or `cpu`.
   ...
   Intel GPU without it, `cpu` if the probe found no GPU. `rocm` is the
   recommended AMD lane — it leads Vulkan on prefill — and requires the
   `/dev/kfd` compute node. `vulkan` needs only a render node, which makes it
   the lane for a box where `/dev/kfd` was never forwarded; an LLM slot on it
   must also resolve a Vulkan-validated runner image, or slot load refuses it
   by name. Either way the refusal is loud and immediate rather than a slot
   that serves nonsense. This sets the slot's
