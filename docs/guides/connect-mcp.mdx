...
(the dashboard's Settings → Secrets panel, or `PUT /api/secrets/{name}`),
...
`env` edits go through `PATCH /api/mcp/{id}/config`. Nothing writes the
`[secrets]` table yet — edit it in the record file directly. Each `[env]` and
`[secrets]` entry is sent to the server as a request header of that name, with
a secret resolved from hal0-api's live environment, so set the secret through
Settings → Secrets (which updates the running process) rather than by editing
`api.env` by hand. A hand edit is picked up by the next registry mutation —
re-run `hal0 mcp expose <id> --hermes` to push it into Hermes's config now.
`hal0 mcp restart <id>` does not help: start/stop/restart answer
`501 mcp.supervisor_unavailable` until the process supervisor ships.
...
shows each one's verdict: `allow`, `gated`, `blocked`, `unknown_tool`, or
`unknown_server`. Verdicts are read from the Hermes seed at
`/etc/hal0/agents/hermes.toml`, which only carries servers that are enabled
and exposed to Hermes — so until you run `hal0 mcp expose <id> --hermes`,
every tool reports `unknown_server`, whatever its policy. A `stdio` server
answers `501 mcp.supervisor_unavailable`.
...
automatically. Once the server is exposed to Hermes, re-run
`hal0 mcp test <id>` to confirm a verdict changed.
...
one-shot patch) and re-probes the server; the probe result is in the REST
response's `hermes_sync.probe` (the CLI prints only the new flags), so run
`hal0 mcp test <id>` to see a bad URL at expose time rather than at the
agent's first turn. `openwebui`/`opencode` are
