...
now** for this bank, and the trash icon deletes the bank. Delete opens a
destructive confirm that asks you to type the bank id before calling
`DELETE /api/memory/banks/{bank_id}`; there is no undo.
...
- **Fact** — free text, a type picker (`world` or `experience` only —
  ...
  tags. Retain is async: after "Fact added", the dashboard follows the
  extraction and raises a warning toast if nothing was learned.
...
- **Mental model** — a name and a standing question. There is no
  refresh-schedule picker.
