...
| Runner key | Runtime | Device class | Backend(s) |
|---|---|---|---|
| `rocmfpx` | llama-server | gpu | rocm, vulkan (the **Standard** default) |
| `promptforge` | llama-server | gpu | rocm |
| `strix` | llama-server | gpu | vulkan |
...
`rocmfpx` (dashboard title **Standard**) is the one image allowed to declare
two backends: the shipped build is Vulkan-portable, so it serves both GPU
lanes on AMD. `vulkanfpx` is a **retired spelling**, not a separate registry
entry — it folds to `rocmfpx` through a permanent alias
(`hal0.runners.RUNNER_ALIASES`) kept only for old TOML/env values and
operator muscle memory. Every other GPU runner declares exactly one backend
by design (ADR-0006): `promptforge` (dashboard title **PromptForge**) is
ROCm-only, and `strix` (dashboard title **Strix**) is Vulkan-only —
qwen4exp-architecture and MTP speculative-decode support, no FPX-family
quants.

A slot with no `binary` set derives one from its `device`
(`hal0.runners.runner_for_backend`) — this is the ordinary case. A
**profile** can optionally pin one of these runner keys too (see [Choose
models → Preferred profile](/docs/guides/choose-models)); flags-only
profiles (all seeds) still never name an image directly — see
...
<Aside type="note" title="Every shipped image is a registry runner (ADR-0006)">
Pin-only images — catalogued for discovery but reachable only through a
slot's `image_pin` escape hatch, never enumerable as a real Runtime option —
are retired as a shipping shape. `strix` is the template: a full registry
entry with its own title, blurb, and declared backend, reachable from the
slot drawer's Runtime select like any other runtime. `image_pin` still
exists, but strictly as the debug-only hatch it was always meant to be — a
custom build, a rollback, or a hand-edited ref — not how an operator
normally reaches a shipped runtime. See
`docs/adr/0006-every-shipped-image-is-a-registry-runner.md` (repo-only) for
...
The `img` (ComfyUI) slot resolves through its own provider instead —
`image_pin`, then a legacy `image` string, then the `comfyui` runner image —
and has no `[slots].default_images` tier. See
...
right-hand card shows the selected row's detail. Rows group under two
headings: **Default runtime** — each family's current release/override
default — and **Optional runtimes** — every other registry entry, with
specialty images (PromptForge, Strix) sorted first. Since ADR-0006 made
every catalogued image a real registry entry, this is a strict two-way
split; there's no longer a third bucket for uncatalogued or pin-only images.
...
writing the set holds two refs: `ghcr.io/hal0ai/hal0-combined:0826`
(`VULKAN_FIXED_IMAGE`, the Standard/`rocmfpx` runtime's Vulkan lane) and
`ghcr.io/hal0ai/hal0-strix-vulkan:0831` (`DEFAULT_STRIX_IMAGE`, the Strix
runtime) — check the constant in `src/hal0/config/schema.py` for the current
membership, since new refs are added only after passing the validation
matrix.
