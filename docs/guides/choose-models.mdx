...
A profile's create/edit form has an optional **Runtime** select — "— Auto ·
no runtime pinned —" to leave it unset, or any runtime this box's registry
carries to pin one (the single-purpose runtimes like Kokoro included, since a
profile has no slot type to narrow the list to one family). A profile
that pins a runtime shows a matching badge on its card. Seed profiles always
render the select disabled: forking a seed copies its runtime binding
verbatim, and you change it by editing the copy. Naming a runtime here does
more than set a default flag bundle — applying the profile to a slot (at
create time, from the slot drawer, or via a model's preferred-profile
adoption) also sets that slot's runtime and, for a single-backend runtime,
its GPU lane. See [Manage slots → Dashboard: the slot edit
drawer](/docs/guides/manage-slots) for what that apply looks like from the
slot side.
...
subsequent model swap. The preference is honoured only when it's compatible
with the slot's existing device *class* (gpu/cpu/npu/img) and type — an
incompatible preference is ignored and the slot keeps its current profile.
Within that same device class, though, a preferred profile that pins a
runtime (see [The Profiles tab](#filter-and-sort-the-catalog) above) *can*
now flip the slot's specific GPU lane — e.g. adopting a Strix-carrying
preference on a `gpu-rocm` slot moves it to `gpu-vulkan` — the same
profile-wins reconcile the slot drawer's Profile row previews. It's still
never adopted if that flip isn't physically possible on this host (a
ROCm-only runtime with no `/dev/kfd`, for instance); the slot keeps its
current profile in that case instead. A flags-only preferred profile (no
runtime pinned) behaves as before and never touches the slot's hardware.
