...
`hal0 update` refuses outright on an editable/dev install (e.g. a `pip
install -e` checkout) — there is no git-based update path. The command dies
early with:

```
hal0 is installed in editable mode from <path>. Install from release wheel with `pip install hal0`.
```

To update a dev box, install from a release wheel with `pip install hal0`, or
rebuild/reinstall the checkout manually.
