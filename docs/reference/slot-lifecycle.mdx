...
| `ERROR` | Failed. Details in `state.json` and journald. A deliberate stop is not a failure: rendered units declare `SuccessExitStatus=143`, a plain SIGTERM stop on an older unit is recognised, and a cached `ERROR` converges to `OFFLINE` once the unit shows a clean stop or `systemctl reset-failed`. A crashed or crash-looping unit keeps its `ERROR`. |
...
| `SeamTimeout` (`slot.seam_timeout`) | 504 | A bounded `systemctl`/podman seam call (Quadlet write, `daemon-reload`, `reset-failed`, the spawning `systemctl restart`) did not return within its budget. `details` carries `command`, `budget_s` and `last_state`; the slot lands in `ERROR` with the same message. |
| `SlotConfigError` | 400 | — |
