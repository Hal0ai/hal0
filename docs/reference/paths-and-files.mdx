...
├── mcp-servers/<id>.toml    # 0600, one record per user-installed MCP server
├── oauth-providers.toml     # OAuth provider registry, seeded once from shipped defaults
...
├── mcp/hermes-managed.json   # MCP server ids hal0 itself joined into Hermes config
