...
`--auto` it always seeds the `agent` and `utility` slots (plus the NPU-trio slots when FLM is present) and seeds capability defaults from the hardware probe — no model
...
`--hardware`/`--type` (`rocm`, `rocm-dense`, `rocm-moe`, `vulkan`, `cuda`,
