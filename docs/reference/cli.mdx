...
| `memory migrate` | Migrate a legacy Cognee store to Hindsight banks (dry-run only), **or** run the Hindsight&lt;-&gt;Honcho engine when `--from`/`--to` are given. | `--from`/`--to hindsight\|honcho`, `--agent`, `--dataset` (repeatable), `--since`, `--resume`, `--dry-run`/`--no-dry-run`, `--cognee-dir`, `--json` |
...
`memory migrate` with no `--from`/`--to` reports the legacy Cognee plan without
writing; apply/write mode is not yet implemented there. With `--from`/`--to`
it runs the Hindsight&lt;-&gt;Honcho backfill/graph-sync engine in-process
against the local hal0-api + Honcho REST surfaces, defaulting to a live run
(`--dry-run` opts into a scan-only report).
