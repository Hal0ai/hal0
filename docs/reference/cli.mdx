...
--apply` requires `hindsight-api>=0.8.0` — as of writing the live engine is
0.7.2, which has no cross-bank document-transfer endpoint, so `--apply`
refuses with a clear error rather than faking a migration via
export/import (which would drop the provenance tags). `--dry-run` (the
default) works on any version since it only reads bank stats.
