...
| `hal0 chat` | Terminal chat REPL over `/v1/chat/completions` — handy for SSH/headless boxes. In-REPL: `/think on\|off\|default`, `/clear`, `/quit` (or Ctrl-D). | `--model` (default `agent`), `--base-url` (default `http://127.0.0.1:8080/v1`), `--no-stream` |
...
| `--no-stream` | Wait for each full reply instead of streaming tokens live. |
