...
| `memory bank profile get <bank>` | Show a bank's profile (name, mission, disposition traits). | `--json` |
| `memory bank profile set <bank>` | Read-modify-write a bank's profile. | `--retain-mission`, `--observations-mission`, `--reflect-mission`, `--skepticism`, `--literalism`, `--empathy`, `--json` |
...
`memory bank profile set` splits its flags across two upstream Hindsight
calls: `--skepticism`/`--literalism`/`--empathy` go through the bank's
`profile` endpoint (disposition traits), while `--retain-mission`/
`--observations-mission`/`--reflect-mission` go through its `config`
endpoint (bank config overrides) — they read like one "profile" but are
different upstream resources. `memory bank delete` requires `--confirm
<bank>` to exactly match the target bank id before it even calls the API;
the server enforces the same rule server-side. `memory migrate unify
--apply` requires hindsight-api's document-transfer API
(`features.document_export_api`/`document_import_api` on `/version`) — as
of writing the live engine is 0.7.2, which lacks it, so `--apply` refuses
with a clear error rather than faking a migration via export/import
(which round-trips a whole bank template and would drop the provenance
tags). `--dry-run` (the default) works on any version since it only reads
bank stats. There is no per-document tag-rewrite option on the transfer
...
import, then a read-merge-write `PATCH` of each new document's tags
(`hal0 memory bank profile`-style: full-replace body, so existing tags are
preserved). Retagging N documents queues N Hindsight re-consolidation
