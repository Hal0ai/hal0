...
| `HAL0_TOOLBOX_IMAGE_<KEY>` — one per `RUNNER_IMAGES` key: `_ROCMFPX`, `_PROMPTFORGE`, `_STRIX`, `_CUDA`, `_CPU`, `_FLM`, `_COMFYUI`, `_KOKORO`, `_MOONSHINE`, `_QWEN3TTS` | unset (release manifest's digest pin, else the bundled default image) | Optional per-runner container image ref overrides — `<KEY>` is the runner's registry key uppercased (`hal0.runners.resolve_runner_image`), not the hardware backend name. Highest-precedence tier of `resolve_runner_image`'s three-tier lookup (env → manifest digest pin → bundled default); shared by every provider that pulls a toolbox image (llama-server's GPU-hardware-gated runners, FLM, ComfyUI, Kokoro, Moonshine, Qwen3-TTS). The legacy alias `HAL0_TOOLBOX_IMAGE_VULKANFPX` still resolves (folds to `_ROCMFPX`, with a one-time deprecation log). `_CPU` names the `cpu` runner's image (`hal0-toolbox-cpu`, wired on #2126); it is also the escape hatch the installer's CPU-only gate points at if that runner ever loses its image again. `_PROMPTFORGE` and `_STRIX` name the two optional (non-default) runners — see [Providers, profiles & devices → Runner images](/docs/reference/providers-profiles-devices/#runner-images). |
...
| `HAL0_BIND_HOST` | `127.0.0.1` when unset; the installer seeds `0.0.0.0` | API bind host. Also read by `hal0 serve --host`. Any value other than `127.0.0.1`/`localhost`/`::1` counts as LAN-bound: with an admin key configured, ADMIN-class routes then require an admin credential from non-loopback callers even while `require_auth` is off (#1822). |
...
| `HAL0_DASHBOARD_URL` | unset → `HAL0_API_URL` → `http://hal0.local:8080` | Base of the OAuth `redirect_uri` hal0-api sends a provider (`<base>/api/oauth/<provider>/callback`) and the dashboard link rendered into Hermes' persona. Must match the redirect URI registered with the provider. |
...
<Aside type="note">
The bundle-tier env vars (`HAL0_HOST_RAM_GB`, `HAL0_BUNDLES_DIR`) back a bundle
picker that is explicitly unwired/deferred — don't imply it's an active
