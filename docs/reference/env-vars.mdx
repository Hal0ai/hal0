...
| `HAL0_ALLOW_CPU_ONLY` | unset (`0`) | `1` proceeds with a CPU-only install (`device = "cpu"` on seeded slots) when no usable GPU lane is detected, instead of failing or prompting. **Gated on there being a CPU runner image (#2126):** it selects `device = "cpu"`, it does not select a CPU image, and the `cpu` runner currently resolves to the Vulkan GPU toolbox. While that is true the installer refuses the CPU-only path rather than completing an install whose slots crash-loop with SIGILL — set `HAL0_TOOLBOX_IMAGE_CPU` to a CPU-built llama-server image to proceed. |
