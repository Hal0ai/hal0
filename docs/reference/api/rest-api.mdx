...
### `/api/models` — FLM (NPU) repo detection

| Method & path | Purpose |
|---|---|
| `POST /api/models/inspect` | Inspect a HuggingFace repo for pullable variants. Now also detects the FastFlowLM (NPU) repo shape — a `config.json` + tokenizer + an `…nx`-suffixed NPU-quant weight directory (e.g. `model.q4nx`) — and surfaces one whole-repo variant flagged `flm`, routed through the `flm pull` path instead of the GGUF/safetensors path. Detection is shape-based (requires the `nx` weight blob), so a plain safetensors/GGUF repo is never misread as FLM. |
