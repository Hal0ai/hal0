...
These classes are fully enforced once `[security].require_auth` is on. While it's off,
the surface is trusted-LAN-open with one exception (since v1.3.0): with an admin key
configured and the API bound past loopback, ADMIN-class requests from a non-loopback
peer still need an admin credential (`_lan_admin_gate` in `src/hal0/api/auth.py`; see
[Authentication](/docs/operate/auth/)). The table below states
...
| `/api/oauth` | Mixed | OAuth passthrough for Hermes skills (see [Run agents](/docs/guides/run-agents/)): `GET /api/oauth/providers`, `POST /api/oauth/{provider_id}/start` (returns `authorize_url`), `GET /api/oauth/{provider_id}/status`, `DELETE /api/oauth/{provider_id}` (best-effort revoke, then delete the token), `POST /api/oauth/{provider_id}/client-secret`, and `GET /api/oauth/{provider_id}/callback`, the provider's redirect target. The callback is meant to be OPEN, guarded by a single-use `state` nonce, but the exposure rule currently matches only the literal template path, so a real callback classifies ADMIN (#2266). Every other route is ADMIN. Tokens and client secrets persist to `/etc/hal0/api.env` and are never returned. |
...
| `/api/status` | CLIENT | Version/build summary. Includes `build_sha`, the short git SHA of the tree the running process started from — `null` on a release (non-git) install. |
