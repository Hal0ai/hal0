...
| `rocm-dense`   | gpu            | `ghcr.io/hal0ai/hal0-rocmfpx:vulkan-minicpm5`                          | on    | ROCmFPX · DENSE · MTP (sustained-decode) |
| `rocm-moe`     | gpu            | `ghcr.io/hal0ai/hal0-rocmfpx:vulkan-minicpm5`                          | on    | ROCmFPX · MOE · MTP (prefill-bound / -dev ROCm0) |
| `vulkan-dense` | gpu            | `ghcr.io/hal0ai/hal0-rocmfpx:vulkan-minicpm5`                          | on    | ROCmFPX · DENSE · MTP (prefill-bound) |
| `vulkan-moe`   | gpu            | `ghcr.io/hal0ai/hal0-rocmfpx:vulkan-minicpm5`                          | on    | ROCmFPX · MOE · MTP (best decode t/s) |
...
Non-GPU profiles (`flm`, `tts`, `cpu-llm`, `comfyui`) omit it and let
`device_class` drive display. Canonical image refs are pinned per release in
`manifest.json` and may differ from these seed defaults.
</Aside>
...
An earlier single `rocm-mtp` profile was split into `rocm-dense` and `rocm-moe`,
then in v0.9.5 the ROCmFPX slugs were consolidated again into today's 2×2 grid
(`rocm-dense`/`rocm-moe`/`vulkan-dense`/`vulkan-moe`, all on the canonical
`vulkan-minicpm5` runner). `rocm` itself is **not** retired — it's a live seed
profile and the default for `gpu-rocm`. There is no loader remap, so a slot
TOML pointing at a slug that no longer exists (`rocm-mtp`, `rocm-dense`,
`rocmfpx-rocm`, `vkfpx-dense`, `vkfpx-moe`) should be repointed to one of the
current profiles.
</Aside>

### MTP flag bundle

A slot's `mtp` field is a tri-state: `true` forces MTP on, `false` forces it
off, and unset (**Auto**) defers to the profile. Under Auto, MTP is effective
only when **both** the profile opts in (`mtp = true` — only
`rocm-dense`, `rocm-moe`, `vulkan-dense`, and `vulkan-moe` do among the seeds)
**and** the model is MTP-eligible (a registry
...
profiles](/docs/concepts/capabilities-and-profiles#mtp-a-model--profile--slot-decision)
...
<Aside type="caution" title="Forcing MTP on an ineligible model">
An explicit `mtp = true` pointed at a model with no MTP heads makes
llama-server refuse to start ("context type MTP requested but model doesn't
contain MTP layers"). Two independent guards defuse exactly that combination:
an update-time migration clears a stale forced-on override left on an
ineligible model, and a swap-time guard clears it if a model swap lands an
ineligible model on a slot still forced On. Both leave force-off and
eligible force-on alone — only unresolvable-ineligible force-on is cleared.
</Aside>
...
model [stacks](/docs/concepts/stacks) use (no secrets, no host paths, an
