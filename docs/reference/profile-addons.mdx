...
- **A profile, not an image.** Profiles carry no image reference at all —
  hardware and image are [slot-owned](/docs/concepts/slots), not profile-owned.
  `ProfileConfig` is a flag bundle plus display
...
hardware-flag screen in `ProfileCatalog.create` still applies, so an envelope
