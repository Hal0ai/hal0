...
<CardGrid>
  ...
  <Card title="Private visibility is enforced on delete" icon="lock">
    In unified-bank mode, the `delete` path applies the same fail-closed ACL as read/search/list: an agent cannot delete another agent's `visibility:private` memories, even by document id. Unresolvable ids are withheld rather than leaked.
  </Card>
