...
<CardGrid>
  ...
  <Card title="Namespaced writes" icon="setting">
    ...
    writes land in the right per-agent namespace rather than a shared pool. The
    header is self-asserted — `private:` isolates cooperating agents from each
    other, it does not defend against a hostile caller. See
    [Security](/docs/concepts/security#what-x-hal0-agent-is-and-is-not).
  </Card>
  <Card title="Destructive ops are audited and gated" icon="approve-check">
    ...
    more than one item in a single call is a **bulk delete**: it enqueues for
    human approval on both `/mcp/admin` and `/mcp/memory` instead of running.
    Deleting an entire bank additionally requires the caller to echo the bank id
    back via `?confirm=<bank_id>`.
  </Card>
</CardGrid>
