...
`hal0 agent install pi`; the driver provisions a minimal profile: the `hal0`
theme, a hal0 model-slot provider (auto-discovered from `/v1/models`) set as
the default, and memory wiring — the `hal0-memory` MCP server plus hindsight
coding-agent memory (auto recall/retain, per-repo banks).
