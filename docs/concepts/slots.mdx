...
- **The slot owns the physical/placement layer** — `device`, `gpu_index`,
  `n_gpu_layers`, `threads`, `binary` (a key into the `RUNNER_IMAGES`
  registry — the dashboard's slot drawer calls this the slot's **Runtime**),
...
Every image hal0 ships is a `RUNNER_IMAGES` registry entry — the default,
dual-lane build (`rocmfpx`, titled **Standard**, the only entry allowed both
a ROCm and a Vulkan lane) plus optional single-lane runtimes such as
`promptforge` (ROCm) and `strix` (Vulkan, built for `qwen4exp`-class models).
Pin-only images — catalogued but reachable only by a raw ref — are retired;
`image_pin` is a debug-only override (a custom build, an A/B test, a
rollback) and never appears as a pickable Runtime option. See [Capabilities
& profiles](/docs/concepts/capabilities-and-profiles) for the full Runtime
picker and how a profile can carry one.
