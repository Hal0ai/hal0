...
supplies the model, context size, and port; the tuned flags come from the
slot's [profile](/docs/concepts/capabilities-and-profiles), and so does the
container image — unless the slot TOML sets its own top-level `image` field,
which wins over the profile's image.
...
2. **Profile opt-in** — does the slot's profile enable MTP at all? Only the
   `rocm-dense`, `rocm-moe`, `vulkan-dense`, and `vulkan-moe` seed profiles do.
...
`hal0/npu` — and generalizes the pattern to any `type: llm` slot with a model bound: a
